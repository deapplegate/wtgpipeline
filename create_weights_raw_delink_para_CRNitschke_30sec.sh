...
for CHIP in ${!#}
do
  cat ${1}/30sec.log | grep "_${CHIP}$3.fits" > ${TEMPDIR}/crw_images_$$
  ...
  cat ${TEMPDIR}/crw_images_$$ |\
  {
    ...
    do
      ...
      if [ ${filter}=="W-C-RC" ]; then
	      ft=15
	      dt=130
      elif [ ${filter}=="W-J-B" ]; then
	      ft=20
	      dt=170
      else
	      echo "this is meant for W-C-RC and W-J-B only, other filters can sometimes make it through the standard way of processing longer exposures"
	      exit 1
      fi
      fwhm=.7
