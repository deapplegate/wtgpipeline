...
def all(subarudir,cluster,DETECT_FILTER,aptype,magtype,location=None):
    ...
    if True:
        ...
        list = ['SUBARU-10_2-1-W-C-RC','SUBARU-10_2-1-W-C-RC','MEGAPRIME-0-1-r','SUBARU-9-4-W-C-RC']
        for filt in list:
...
def fit(table, combo_dict, instrument_to_locus, magtype, locus_mags, locus_pairs, 
    ...
    for iteration in cycles:       
        ...
        def errfunc(pars,residuals=False,savefig=None):
            ...
            if residuals:
                end_of_locus = scipy.array([reduce(lambda x,y: x*y, [ds.min(axis=1)[i] != ds[i][x] for x in range(10)]) for i in range(len(ds.min(axis=1)))]) 
                print end_of_locus
        ...
        ''' difference: median hold instrumental magnitude - median hold locus magnitude ''' 
        #hold_mag_difference = scipy.median(A_band[:,0,0]) - locus_mags[len(locus_mags)/2][instrument_to_locus[combo_dict['hold']]]
