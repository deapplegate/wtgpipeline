...
def select_analyze():
    ...
    while len(results) > 100 or first:
        ...
        command= "SELECT * from illumination_db where fixchips=0 and OBJNAME like '%0850%'" #(pasted_cat is null or pasted_cat like '%None%' or CRVAL1ASTROMETRY_2 is null) ORDER BY RAND()" # and PPRUN='2003-04-04_W-C-IC'"       
        command = "SELECT * from illumination_db where (OBJNAME like 'MACS0417%' or OBJNAME like 'MACS0025%') and file not like '%CALIB%' and resam is null" # and resamlen is null" # and PPRUN='2006-12-21_W-J-B'"

        #command = "SELECT * from illumination_db where (OBJNAME like 'MACS0025%' ) and file not like '%CALIB%' and resam is null" # and resamlen is null" # and PPRUN='2006-12-21_W-J-B'"
...
def analyze(SUPA,FLAT_TYPE,params={}):
    ...
    try:
        ...
        #raw_input()                
        #if string.find(str(params['fwhm']),'None') != -1:
        #    find_seeing(SUPA,FLAT_TYPE)      
        #get_files(SUPA,FLAT_TYPE)
...
def get_files(SUPA,FLAT_TYPE=None):    
    ...
    for image in [search_params['file']]:
        ...
        params['fil_directory'] = res2[0]
        if 0:
            pattern = "/%(path)s/%(fil_directory)s/SCIENCE/coadd_%(OBJNAME)s*" % params 
            print pattern
            import glob
            file = glob.glob(pattern)
            print file[0]
            if string.find(file[0],'_all') != -1:
                COADDSUFFIX = '_all'
            else: COADDSUFFIX = ''                
            params['COADDSUFFIX'] = COADDSUFFIX
            save_exposure(params,params['SUPA'],params['FLAT_TYPE'])

        params['COADDSUFFIX'] = '_all' 
        save_exposure({'COADDSUFFIX':params['COADDSUFFIX']},params['SUPA'],params['FLAT_TYPE'])
        im = "/%(path)s/%(fil_directory)s/SCIENCE/coadd_%(OBJNAME)s%(COADDSUFFIX)s/%(ROOT)s.sub.%(OBJNAME)s%(COADDSUFFIX)s.resamp.fits" % params
        import glob
        print glob.glob(im)
        print im
    ...
    pattern = res[0] + '_*O' + res[1].replace('S.','S*.')
    print pattern, dict['file'], dict['PPRUN']
    ...
    for ele in dict.keys():
        ...
        for l in letters:
            if string.find(str(dict[ele]),l) != -1: 
                type = 'string'
        if len(str(dict[ele])) == 0: type='string'
        if type == 'float':  
...
def length(SUPA,FLAT_TYPE):
    ...
    save_exposure({'resamlen':'1','crfixed':'third','LENGTH1':LENGTH1,'LENGTH2':LENGTH2,'CRPIX1ZERO':crpixzero['CRPIX1'],'CRPIX2ZERO':crpixzero['CRPIX2'],'CRVAL1':crpix['CRVAL1'],'CRVAL2':crpix['CRVAL2']},SUPA,FLAT_TYPE)

    return chips
    #return x,y

    #command = 'ldaccalc -i ' + search_params['pasted_cat'] + ' -o ' + params['TEMPDIR'] + params['ROOT'] + '.newpos -t OBJECTS -c "(Xpos + ' +  str(float(search_params['CRPIX1ZERO']) - float(crpix['CRPIX1'])) + ');" -k FLOAT -n Xpos_ABS "" -c "(Ypos + ' + str(float(search_params['CRPIX2ZERO']) - float(crpix['CRPIX2'])) + ');" -k FLOAT -n Ypos_ABS "" -c "(Ypos*0 + ' + str(params['NUM']) + ');" -k FLOAT -n CHIP "" ' 
#    print command
#    utilities.run(command)
...
def sextract(SUPA,FLAT_TYPE):
        ...
        for image in search_params['files']:
            ...
            if not child:
                try:
                    ...
                    weightim = "/%(path)s/%(fil_directory)s/SCIENCE/coadd_%(OBJNAME)s%(COADDSUFFIX)s/%(ROOT)s.sub.%(OBJNAME)s%(COADDSUFFIX)s.resamp.weight.fits" % params
                    #flagim = "/%(path)s/%(fil_directory)s/WEIGHTS/globalflag_%(NUM)s.fits" % params
                    #finalflagim = TEMPDIR + "flag_%(ROOT)s.fits" % params 
                    params['finalflagim'] = weightim
                    im = "/%(path)s/%(fil_directory)s/SCIENCE/coadd_%(OBJNAME)s%(COADDSUFFIX)s/%(ROOT)s.sub.%(OBJNAME)s%(COADDSUFFIX)s.resamp.fits" % params
                    ...
                    if True:
                        ...
                        for double in doubles_raw:
                            ...
                            if True: #len(file) > 0:
                                ...
                                command = "nice sex %(im)s -c %(PHOTCONF)s/phot.conf.sex \
    ...
    pasted_cat = path + 'PHOTOMETRY/ILLUMINATION/' + 'pasted_' + SUPA + '_' + search_params['filter'] + '_' + str(search_params['ROTATION']) + '.resam.cat'
    print pasted_cat
    ...
    if len(newposlist) > 1:
        ...
        utilities.run(command)
    save_exposure({'pasted_cat':pasted_cat,'resam':1},SUPA,FLAT_TYPE)
