...
./parallel_manager.sh ./create_science_weighted.sh ${SUBARUDIR}/${cluster}/${filter}_${run} SCIENCE WEIGHTS ${ending} 2>&1 | tee -a OUT-create_science_weighted_${cluster}_${filter}_${run}.log
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
    exit ${exit_stat};
fi
