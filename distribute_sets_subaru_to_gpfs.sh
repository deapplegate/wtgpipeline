#!/bin/bash
set -xv
#adam-example# ./distribute_sets_subaru_to_gpfs.sh ${SUBARUDIR} ${run}_${filter}/SCIENCE ${ending} 1000 ${SUBARUDIR}/SUBARU.list WEIGHTS
...
outputdir=/gpfs/slac/kipac/fs1/u/awright/SUBARU/
...
cat ${TEMPDIR}/images_$$ |\
{
  ...
  do
    ...
    if [ ! -d "/${outputdir}/${nick}" ]; then
      mkdir "/${outputdir}/${nick}"
    fi

    if [ ! -d "/${outputdir}/${nick}/${RUNDIR}" ]; then
      mkdir "/${outputdir}/${nick}/${RUNDIR}"
    fi

    if [ ! -d "/${outputdir}/${nick}/${RUNDIR}/SCIENCE" ]; then
      mkdir "/${outputdir}/${nick}/${RUNDIR}/SCIENCE"
    fi

    if [ ! -d "/${outputdir}/${nick}/${RUNDIR}/SCIENCE/SPLIT_IMAGES" ]; then
      mkdir "/${outputdir}/${nick}/${RUNDIR}/SCIENCE/SPLIT_IMAGES"
    fi
    
    if [ -L ${NAME} ]; then                                                                                                                                                       
        NAMEfile=`readlink -f /$1/$2/${NAME} `
    else
	NAMEfile="/$1/$2/${NAME}"
    fi
    cp ${NAMEfile} /${outputdir}/${nick}/${RUNDIR}/SCIENCE/ 

    split_image=`basename ${NAME} "${3}.fits"`
    ln -s /$1/$2/SPLIT_IMAGES/${split_image}.fits /${outputdir}/${nick}/${RUNDIR}/SCIENCE/SPLIT_IMAGES

    if [ ! -d "/${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}" ]; then
      mkdir "/${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}"
    fi

    if [ ! -f "/${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}/globalweight_2.fits" ]; then
	cp /$1/$2/../${WEIGHTSDIR}/globalweight_*.fits /${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}
	cp /$1/$2/../${WEIGHTSDIR}/globalflag_*.fits /${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}
    elif [ "/$1/$2/../${WEIGHTSDIR}/globalweight_2.fits" -nt "/${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}/globalweight_2.fits" ]; then
	cp /$1/$2/../${WEIGHTSDIR}/globalweight_*.fits /${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}
	cp /$1/$2/../${WEIGHTSDIR}/globalflag_*.fits /${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}
    else
	echo "/${outputdir}/${nick}/${RUNDIR}/${WEIGHTSDIR}/globalweight_2.fits already up-to-date"
    fi
