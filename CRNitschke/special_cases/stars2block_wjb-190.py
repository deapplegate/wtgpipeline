...
def getstar_segments(star_infl,seeing=None,radmagrange=None,mode='liberal',unsat_cut=None,**kwargs): #,mode="liberal",unsat_cut=None,ELLIPTICITY=.25)
	...
	star_only_catfl=savedir+"CATALOG_CRN-stars_%s_%s.%s_wjb-190.cat" % (OBJECT,FILTER,flname,)
	...
	star_only_segfl=savedir+"SEGMENTATION_CRN-stars_%s_%s.%s_wjb-190.fits" % (OBJECT,FILTER,flname,)
        hdu=pyfits.PrimaryHDU(segments_stars)
        hdu.writeto(star_only_segfl ,clobber=True)
