...
def match_OBJNAME_specific(OBJNAME,FILTER,PPRUN):
    ...
    #    if 1: #a[0] == 'y':                                             
            ...
            #def linear_fit(OBJNAME,FILTER,PPRUN,cov=None,CONFIG=None,true_sdss=None,CAT_TYPE=None):
            linear_fit(OBJNAME,FILTER,PPRUN)
...
def match_OBJNAME(OBJNAME=None,FILTER=None,PPRUN=None):
    ...
    while True: 
        ...
        if OBJNAME is None or start == 0:
            ...
            command="  select i.* from temp i left join fit_db f on (i.pprun=f.pprun and i.OBJNAME=f.OBJNAME) where i.PPRUN !='KEY_N/A' and i.file not like '%CALIB%' and i.pprun like '%2000%' and (f.linearfit=1) GROUP BY i.pprun,i.filter,i.OBJNAME ORDER BY RAND() " # and PPRUN='2006-12-21_W-J-B' GROUP BY OBJNAME,pprun,filter"
        ...
        if len(f_results) > 0: 
            ...
            try: 
                ...
                primary, secondary, good_list, help_list = sort_results(results,dtop,db_keys)
                print good_list
                ''' match images '''
                print primary, secondary
                #raw_input()
                finalcat = match_many_multi_band([[good_list[primary]['catalog'],'primary'],[good_list[secondary]['catalog'],'secondary']])
                print finalcat
...
def sort_results(results2,dict,db_keys):
    ...
    for line in results2: 
        ...
        if float(dict['EXPTIME']) > 10.0:
            if not dict['PPRUN'] in rotation_runs:                                                                                                                               
                rotation_runs[dict['PPRUN']] = {'ROTATION':{dict['ROTATION']:'yes'},'FILTER':dict['filter'],'CONFIG_IM':CONFIG_IM,'EXPTIME':dict['EXPTIME'],'file':dict['file'],'linearfit':dict['linearfit'],'OBJNAME':dict['OBJNAME'],'catalog':dict['catalog'],'FILTER_NUM':FILTER_NUM}
            rotation_runs[dict['PPRUN']]['ROTATION'][dict['ROTATION']] = 'yes'
    ...
    print 'orphaned'
    for key in sorted(orphan_list.keys()): print key, orphan_list[key]['EXPTIME'],orphan_list[key]['file']
    return primary, secondary, good_list, help_list
...
def linear_fit(OBJNAME,FILTER,PPRUN,cov=None,CONFIG=None,true_sdss=None,primary=None,secondary=None):
    ...
    save_fit({'PPRUN':PPRUN,'FILTER':FILTER,'OBJNAME':OBJNAME,'primary_filt':primary,'secondary_filt':secondary,'sdss_coverage':str(cov),'relative_colors':relative_colors,'true_sdss':true_sdss,'CONFIG':CONFIG})

    if not quick:
        EXPS, star_good,supas = selectGoodStars(EXPS,cov,LENGTH1,LENGTH2,relative_colors)               
