...
########				echo "check: doing " ${run}_${filter}/${FLAT} " if this is SKYFLAT, make sure the median counts are pretty close" >> adam_look-flat_imstats_group2.log
########				imstats ${SUBARUDIR}/${run}_${filter}/${FLAT}/SUPA*3.fits >> adam_look-flat_imstats_group2.log
########				imstats ${SUBARUDIR}/${run}_${filter}/${FLAT}/SUPA*8.fits >> adam_look-flat_imstats_group2.log
########
########				#if [ "$config" == "10_3" ] ; then
...
########				echo "adam-look| ds9 -zscale -geometry 2000x2000 ${SUBARUDIR}/${run}_${filter}/${FLAT}/BINNED/*mosOC.fits -zoom to fit "
...
########	do
########		if [ "$filter" == "W-S-I+" ] ;then #adam-tmp#
########			if [ "${run}" == "2007-02-13" ] ;then #adam-tmp#
########				continue #adam-tmp#
########			fi #adam-tmp#
########		fi #adam-tmp#
########		for FLAT in "DOMEFLAT" "SKYFLAT"
...
#if_OC_fls# ### link science data back into SCIENCE
#if_OC_fls# if [ ! -d ${SUBARUDIR}/${run}_${filter}/SCIENCE/OC_IMAGES ]; then
#if_OC_fls#     mkdir ${SUBARUDIR}/${run}_${filter}/SCIENCE/OC_IMAGES
#if_OC_fls# fi
#if_OC_fls# mv ${SUBARUDIR}/${run}_${filter}/SCIENCE/SUPA*OC.fits ${SUBARUDIR}/${run}_${filter}/SCIENCE/OC_IMAGES
...
########################################################
#adam-new#
#adam-SHNT# So, lots of the stuff from here down will have to be changed (for example all of the stuff with the rename commands)
