...
cat ${TEMPDIR}/coaddimages_$$ |\
{
  ...
  do
    ...
    #ROTATION=`${P_DFITS} ${file} | ${P_FITSORT} ROTATION |\
    #        ${P_GAWK} '($1!="FILE") {print $2}'`
    ...
    if [ "${BADCCD}" != "1" ] && [ "${ASTBAD}" != "1" ]; then
      ...
      if [ "${EXTERNHEAD}" -eq 1 ]; then
        ...
        #grep -v "^CD\|END" /${MAIN}/${SCIENCE}/${EXTERNHEADDIR}/${HEADBASE}.head > \
        #  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	if [ "$?" -gt "0" ]; then exit $? ; fi
        #if [ "${ROTATION}" -eq 1 ]; then
	#	echo "CD1_1   = -${PXSCALE}" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD2_2   = ${PXSCALE}" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD2_1   = 0.000000000000" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD1_2   = 0.000000000000" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
        #elif [ "${ROTATION}" -eq 0 ]; then
	#	echo "CD2_1   = ${PXSCALE}" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD1_2   = ${PXSCALE}" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD1_1   = 0.000000000000" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#	echo "CD2_2   = 0.000000000000" >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	#fi
	#echo "END     " >>  ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
...
if [ "${HEADER}_A" == "_A" ]; then
  ...
  if [ "$?" -gt "0" ]; then exit $? ; fi
  mv ./coadd.head ${HEADDIR}/
  #adam: make sure that ${HEADDIR}/coadd_${COADDFILENAME}.head (the swarp output) is consistent with the input. Nothing related to the CDELT[1-2] can be overwritten.

else
  echo "adam-look prepare_coadd_swarp.sh: on the last else where I believe the SWARP output must be modified to include CDELT1 and CDELT2"
  ...
  #adam-SHNT# swarp stuff reverted back to old version
  ${P_SWARP} -c ${DATACONF}/create_coadd_swarp.swarp \
             -RESAMPLE N -COMBINE N \
             -HEADER_ONLY Y \
	     @${TEMPDIR}/files_$$.list
  ...
  #adam-old#   grep -v "^CDELT" > ${HEADDIR}/coadd_${COADDFILENAME}.head
  fold coadd.fits | grep -v "^PV\|^CDELT[1-2]" > ${HEADDIR}/coadd_${COADDFILENAME}.head
  if [ "$?" -gt "0" ]; then exit $? ; fi

  cp ${HEADER} ./coadd.head
  #adam-old#rm -f coadd.head
fi
