...
class CartesianUpdater(Updater):
    ...
    def gatherFilesys(self):
        filename = parallel_filename_base + self.pid + self.uid + parallel_filename_ext
        self.save(filename)
        total = 0
        ...
        j = 0
        for filename in glob.iglob(parallel_filename_base + '*' + self.uid + parallel_filename_ext):
            try:
                f = open(filename, 'rb')
                s = pickle.load(f)
                f.close()
...
The module should be very flexible, but is designed with these things foremost in mind:
  ...
  2. making is straightforward to break the parameter space into subspaces which can be sampled using different proposal methods and at different rates. For example, if changing some parameters requires very expensive calulations in the likelihood, the other, faster parameters can be sampled at a higher rate. Or, some parameters may lend themselves to Gibbs sampling, while others may not, and these can be block updated independently.
...
Real-valued parameters are assumed, but the framework can be used with other types of parameters, with suitable overloading of classes.

A byproduct of item (1) is that the user is expected to handle all aspects of the calculation of the posterior. The module doesn't implement assignment of canned, standard priors, or automatic discovery of shortcuts (like Gibbs sampling). The idea is that the user is in the best position to know how the details of the likelihood and priors should be implemented.

It's probably worth reading the docstrings for:
 Parameter
 ParameterSpace
 Updater, CartesianSequentialUpdater, CartesianPermutationUpdater, MultiDimSequentialUpdater, MultiDimPermutationUpdater
 Slice, Metropolis 
...
Here is a quick overview of the class structure:
 ...
 Updater objects provide the mechanism to sample a ParameterSpace. They come in two varieties:
...
The definition of an MCMC iteration in this implementation can be a little confusing. As far as an MultiDim updater object's 'count' attribute (controling the timing of adaptations, for example) is concerned, an iteration corresponds to one proposal along some direction. For Cartesian updaters, each iteration corresponds to a separate proposal for all of the Parameters in the Updater's ParameterSpace. (This disparity is unfortunate, but ensures that the count attribute is the correct value to use when, e.g., calculating chain vairnaces on the fly.) In either case, the Updater's rate attribute determines how many of these iterations occur each time the updater is called by the Engine (default=1). However, as far as the Engine is concerned, an iteration corresponds to a loop over all loaded Updaters. So if there are two MultiDim updaters, u1 and u2, with rates set to 1 and 2, and the Engine's updater list is [u2, u1, u2] (repetition is perfectly allowed, and sometimes desirable), then each iteration of the Engine actually corresponds to 1*u1.rate + 2*u2.rate = 5 proposals. However, if u1 is instead a Cartesian updater controlling 3 Parameters, then each engine iteration implies 7 proposals. The chain is stored to the Backends after each Engine iteration. The upshot of all this is that there's lots of flexibility to sample parameters at different rates and/or thin the chain as it's being run, at the expense of a little complexity.
