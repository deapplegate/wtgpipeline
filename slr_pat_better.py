...
def mkcolorcolor(filt,catalog,starcatalog,cluster,magtype,save_file=None):
    ...
    for iteration in ['full']: #,'bootstrap1','bootstrap2','bootstrap3','bootstrap4']:       
        ...
        A_band = scipy.swapaxes(scipy.swapaxes(scipy.array(loci*[[table.field('MAG_' + magtype + '-' + a[0]) for a in filters_fs]]),0,2),1,2)
        A_err = scipy.swapaxes(scipy.swapaxes(scipy.array(loci*[[table.field('MAGERR_' + magtype + '-' + a[0]) for a in filters_fs]]),0,2),1,2)
        ...
        def errfunc(pars,residuals=False):
            ...
            if 1:                
                A_zp = scipy.swapaxes(scipy.swapaxes(scipy.array(loci*[stars_good*[[assign_zp(a[1],pars,zps) for a in filters_fs]]]),0,1),0,0)
                #print A_zp.shape
                #raw_input()
                ...
                A_band_average = scipy.swapaxes(scipy.swapaxes(scipy.array(len(filters_fs)*[average]),0,2),0,1)
                ...
                dotprod = abs((A_band - A_band_average - locus_matrix - A_zp) * A_err)
