...
def shear_ratio(cluster_z):    

  d_cluster = calc_ang(cluster_z)


def dist_ratio(zs,cluster_z=0.55):
  import pylab
  #cluster_z = 0.55
  ratios = []
  for z in zs:             
    d_cluster = calc_ang(cluster_z)
    ratios.append((calc_ang(z) - d_cluster)/calc_ang(z))
...
if __name__ == '__main__':
  ...
  ratios = []
  zs = []
  for z in scipy.arange(cluster_z,4,0.2):
    zs.append(z)
    ratios.append((calc_ang(z) - d_cluster)/calc_ang(z))
