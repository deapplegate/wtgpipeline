...
def hist2d_matrix(frame, alpha=0.5, figsize=None, ax=None, grid=False,
                   diagonal='hist', marker='.', density_kwds=None,
                   hist_kwds=None, range_padding=0.04, bins=None,**kwds):
    """
    ...
    for i, a in zip(lrange(n), df.columns):
        for j, b in zip(lrange(n), df.columns):
            ...
            if i == j:
                ...
                if diagonal == 'hist':
                    ax.hist(values, bins=bins[j],**hist_kwds)
		...
		import matplotlib.cm as cm
	        #imshow(im,origin='lower left', interpolation='nearest',norm=LogNorm(vmin=1,vmax=128))
                ax.hist2d(df[b][common], df[a][common],bins=[bins[j],bins[i]],norm=LogNorm(), **kwds)
