...
output_catalog = '%(SUBARUDIR)s/%(CLUSTER)s/%(PHOTOMETRYDIR)s/%(CLUSTER)s.APER1.%(iaper)s.photoz.CWWSB_capak.list.slr.tab' % dict 

print output_catalog
p = pyfits.open(output_catalog)['STDTAB'].data
import scipy 
...
print a
raw_input()
a, b, varp = pylab.hist(a,bins=scipy.arange(0,4,0.02))
pylab.xlabel("Z")
pylab.ylabel("Number of Galaxies")
pylab.show()
