...
${P_LDACCALC} -i ${TEMPDIR}/tmp2_$$.cat -o ${TEMPDIR}/tmp21_$$.cat -c "(EXPTIME);" \
	      ...
	      -c "(CRVAL2);" -n "DEC" "" -k FLOAT -t FIELDS -r STATS \
              -x EXPOSURE -s mean\
	      -c "(ROTATION);" -n "ROTATION" "" -k FLOAT -t FIELDS -r STATS \
              -x EXPOSURE -s mean\
	      -c "(EXPOSURE);" -n "EXPOSURE" "" -k FLOAT -t FIELDS -r STATS \
              -x EXPOSURE -s mean
...
${P_LDACCALC} -i ${TEMPDIR}/tmp3_$$.cat -o /$1/$2/cat/chips.cat5 -c "(SEXSFWHM);" \
              -n "SEEINGSE_SDEV" "" -k FLOAT -t FIELDS -r STATS \
...
PXSCALE=`${P_LDACTOASC} -i /$1/$2/cat/chips.cat5 -t FIELDS -b -k CD1_1 CD2_1 | ${P_GAWK} '{if(NR==1) print 3600.0*sqrt($1*$1+$2*$2)}'`
...
${P_LDACCALC} -i ${TEMPDIR}/tmp9_$$.cat -o ${TEMPDIR}/tmp10_$$.cat -c "(e1);" \
	      ...
	      -c "(e2);" -n "e2" "e2_starellipticity" -k FLOAT \
              -n "seeing_fwhmse" "seeing from SE FWHM" -k FLOAT -c "(FWHM_IMAGE*${PXSCALE});" \
              -n "seeing_rh_al" "seeing from rh*4.544" -k FLOAT -c "((rh*1.81)*${PXSCALE});" \
              -t OBJECTS -r STATS \
              -x EXPOSURE -s median
...
####################################################
### also add a CHIPS_STATS table, with information
### on a chip-by-chip basis
####################################################

${P_LDACRENTAB} -i /$1/$2/cat/chips.cat6 \
                -o ${TEMPDIR}/tmp100_$$.cat \
                -t FIELDS CHIPS_STATS

${P_LDACADDTAB} -i /$1/$2/cat/chips.cat6 \
                -o ${TEMPDIR}/tmp101_$$.cat \
                -t CHIPS_STATS \
                -p ${TEMPDIR}/tmp100_$$.cat \

# add statistical information seeing and sky background:
${P_LDACCALC} -i ${TEMPDIR}/tmp101_$$.cat \
              -o /$1/$2/cat/chips.cat7 \
              -t CHIPS_STATS \
              -n "SEEING" "" -k FLOAT \
              -c "(SEXSFWHM);"
...
${P_LDACFILTER} -i /$1/$2/cat/chips.cat7 -o ${TEMPDIR}/tmp103_$$.cat -c "(cl=2);"

${P_LDACCALC} -i ${TEMPDIR}/tmp103_$$.cat -o ${TEMPDIR}/tmp104_$$.cat \
              -n "e1" "e1_starellipticity" -k FLOAT -c "(e1);" \
	      -n "e2" "e2_starellipticity" -k FLOAT -c "(e2);" \
              -n "e_abs" "" -k FLOAT -c "(sqrt((e1*e1)+(e2*e2)));" \
              -t OBJECTS -r CHIPS_STATS \
              -x FIELD_POS -s median

${P_LDACCALC} -i ${TEMPDIR}/tmp104_$$.cat -o ${TEMPDIR}/tmp105_$$.cat \
              -n "seeing_fwhmse" "seeing from SE FWHM" -k FLOAT -c "(FWHM_IMAGE*${PXSCALE});" \
              -n "seeing_rh_al" "seeing from rh*4.544" -k FLOAT -c "((rh*1.81)*${PXSCALE});" \
              -t OBJECTS -r CHIPS_STATS \
              -x FIELD_POS -s median

${P_LDACCALC} -i ${TEMPDIR}/tmp105_$$.cat -o /$1/$2/cat/chips.cat8 \
              -n "e1_SDEV" "e1_starellipticity" -k FLOAT -c "(e1);" \
	      -n "e2_SDEV" "e2_starellipticity" -k FLOAT -c "(e2);" \
              -n "e_abs_SDEV" "" -k FLOAT -c "(sqrt((e1*e1)+(e2*e2)));" \
              -t OBJECTS -r CHIPS_STATS \
              -x FIELD_POS -s stddev

${P_LDACTOASC} -i /$1/$2/cat/chips.cat8 \
               -t CHIPS_STATS \
               -s -b -k EXPOSURE CONFIG IMAGEID \
               > ${TEMPDIR}/tmp_id_list_$$.dat
...
${P_GAWK} '{
            ...
            detid=$3
            type=0
            while (getline < ("'${REDDIR}'/chip_types_c"config".dat") > 0)
            {
              if($1==detid)
              {
              type=$2
            ...
            close(("'${REDDIR}'/chip_types_c"config".dat"))
            print expo, detid, type
           }' ${TEMPDIR}/tmp_id_list_$$.dat > ${TEMPDIR}/tmp_ccd_type_$$.dat

${P_ASCTOLDAC} -i ${TEMPDIR}/tmp_ccd_type_$$.dat \
               -o ${TEMPDIR}/tmp_ccd_type_$$.cat \
               -t CHIPS_STATS \
               -c ${REDDIR}/ccd_type.a2l.conf

${P_LDACJOINKEY} -i /$1/$2/cat/chips.cat8 \
		 -o /$1/$2/cat/chips.cat9 \
		 -t CHIPS_STATS \
