...
'''
sql = '\
create table slrsdss( id INT NOT NULL AUTO_INCREMENT,\
PRIMARY KEY(id),\
objname varchar(30),\
gfaint float,\
rfaint float,\
...
def run_slrsdss(OBJNAME=None,fix=False):
    ...
    while loop:        
        ...
        if OBJNAME is None and not fix:
            ...
            command='SELECT * from slrsdss where OBJNAME="' + OBJNAME + '"' 
            loop = False
        ...
        if False: #len(results) > 0:
            ...
            logfile = os.environ['sne'] + '/slr_log/' + dtop2['objname']
            import os
...
def distributions():
    ...
    ''' need to retrieve average correction for each filter '''
...
def compute_illum():
    ...
    for filt in filters:
        d = {}                                                                                                                                                                                                                                                                                                                             
        fit_num = {}
        for positioncolumn in positioncolumns:
            command = 'select ' + positioncolumn + ' from test_fit_db as f join test_try_db as t on (f.objname=t.objname and f.pprun=t.pprun) where t.sdss_var_correction < 0.01 and t.todo="good" and f.sample="SDSS" and f.sample_size="all" and f.1$0x1y is not null and f.0$0x1y is not null and (f.config="10_1" or f.config="10_2") and t.filter="' + filt + '" '

            #command = 'select ' + positioncolumn + ' from test_fit_db as f join test_try_db as t on (f.objname=t.objname and f.pprun=t.pprun) where t.todo="good" and f.sample_size="all" and (f.config="10_1" or f.config="10_2") and f.1$3x3y is not null and f.0$0x2y is not null and t.filter="' + filt + '" '

            #command = 'select ' + positioncolumn + ' from test_fit_db as f join test_try_db as t on (f.objname=t.objname and f.pprun=t.pprun) where t.todo="good" and f.sample_size="all" and (f.config=9.0) and f.0$0x2y is not null and t.filter="' + filt + '" '

            #command = 'select ' + positioncolumn + ' from test_fit_db as f join test_try_db as t on (f.objname=t.objname and f.pprun=t.pprun) where t.sdss_var_correction < 0.03 and f.sample="SDSS" and f.sample_size="all" and (f.config=9.0) and t.filter="' + filt + '"'
            print command        
            c.execute(command)
            results=c.fetchall()
            print results
            print [x[0] for x in results]
            #results = [float(str(x[0]).replace('None','0')) for x in results]
            results = [float(str(x[0])) for x in results]
            import scipy
            print scipy.mean(results)
            d[positioncolumn] = scipy.mean(results)
            fit_num[positioncolumn] = len(results)
        ...
        for ele in position_columns_names:                      
            ...
            if string.find(ele['name'],'zp') != -1 and string.find(ele['name'],'image') == -1:
        ...
        diff = ((x-LENGTH1/2.)**2.+(y-LENGTH2/2.)**2.)**0.5 - 60*13/0.2 #(LENGTH1/2.)**2.
...
def test_plot():
    ...
    fig_height = fig_width*0.1*golden_mean      # height in inches
...
def calcPlotPublish(input): #file, LENGTH1, LENGTH2, data,magErr, X, Y, pth='/nfs/slac/g/ki/ki04/pkelly/plots/', rot=0, good=None, limits=[-0.4,0.4], ylab='SUBARU-SDSS'):
    ...
    for i in range(len(data)):
        ...
        diff = ((X[i]-LENGTH1/2.)**2.+(Y[i]-LENGTH2/2.)**2.) - (LENGTH1/2.)**2.

        if use and diff < 0:
            if LENGTH2*0.2 < Y[i] < LENGTH2*0.8 and magErr[i] < 0.15:                                                                                             
                X_cen.append(X[i])
                Y_cen.append(Y[i])
                import scipy
    ...
    z_p = scipy.array(data_cen) - scipy.median(scipy.array(data_cen)) # subtract MEDIAN
    ...
    pylab.clf()
    pylab.axes([0.125,0.2,0.95-0.125,0.95-0.2])
    pylab.axis((x[0],x[-1], limits[0], limits[1]))
    print pylab.axis()
    #pylab.ylim(limits)
    #pylab.xlim(x[0],x[-1])
    pylab.scatter(x_p,z_p,linewidth=0,s=3,marker='o',c='gray')
    pylab.xlabel(r'Column (pixel)')
    pylab.ylabel(r'SDSS-Subaru (mag)')     # label the plot
    pylab.axhline(0,c='black')
    #pylab.figtext(0.16,0.82,'CORRECTED')
    #pylab.subplot(212)
    #pylab.scatter(y_p,z_p,linewidth=0,s=0.2,marker='o')
    #pylab.ylim(limits)
    #pylab.xlim(y[0],y[-1])
    #pylab.xlabel('Y axis')
    ...
    #pylab.subplot(211)
    #pylab.xlabel('X axis')
    #pylab.ylabel('SDSS-SUBARU (mag)')     # label the plot
    #pylab.scatter(mean_X,mean_flat,linewidth=0,s=0.2)
    #pylab.errorbar(mean_X,mean_flat,err_flat,lw=0.)
    #pylab.ylim(limits)
    #pylab.xlim(x[0],x[-1])
    #pylab.subplot(212)
    #pylab.scatter(mean_Y,mean_flat,linewidth=0,s=0.2)
...
def photocalibrate(cluster,mode='SLR',aptype='_aper',test_zps=False, only_first=False, filt=None):
    ...
    if cluster == 'COSMOS_PHOTOZ':
        code = os.system('python ' + os.environ['bonn'] + '/calibrate_COSMOS.py ' + mode + ' ' + aptype + ' ' + str(test_zps))
        if code: raise Exception
        ...
        if len(lensing_bands) == 0: raise Exception

        #lensing_bands = get_lensing_filts(os.environ['subdir'], cluster)       

        #if only_first and len(lensing_bands) > 1: bands = lensing_bands[0]
        #else: bands = lensing_bands
        ...
        for DETECT_FILTER in lensing_bands:
            ...
            if mode == 'SLR':
                ...
                slr_out_iso = slr_catalog_dir_iso + '/' + cluster + '.sedzp.cat'

            print 'running photocalibrate_cat ' + DETECT_FILTER
            #command = os.environ['bonn'] + '/photocalibrate_cat.py -i %(all_phot_cat_iso)s -c %(cluster)s -o %(slr_out_iso)s -t slr %(photocalibrate_cat_flag)s' % {'cluster':cluster, 'all_phot_cat_iso':all_phot_cat, 'slr_out_iso':slr_out_iso, 'photocalibrate_cat_flag':photocalibrate_cat_flag}
            #os.system(command)
...
def add_photoz_column():
    ...
    f_photoz = open('/u/ki/dapple/nfs/pipeline/wtgpipeline/worklist','r').readlines()
    ...
    #f_photoz = open('/u/ki/dapple/nfs/pipeline/wtgpipeline/referenceset','r').readlines()
...
def mk_mag_page(OBJNAME=None,fix=False):
    ...
    web = open(os.environ['sne'] + '/magnification/index.html' , 'w',0)
    ...
    for result in results: 
        ...
        index += 1
        filters = reduce(lambda x,y: x + ' ' + y, filter(lambda x: string.find(x, 'CALIB')==-1, re.split('\s+',dtop2['info'])[2:]))
        cls.append([len(filters),'<tr class=' + class_name + '><td>' + cluster_redshift + '</td><td><a href=' + dtop2['objname'] + '/index.html> ' + dtop2['objname'] + '</a></td></tr>\n'])

        from glob import glob                                                                           

    cls.sort()
    cls.reverse()
    web.write(reduce(lambda x,y: x + y, [z[1] for z in cls]))
    web.write('</table>')
    web.close()        
...
def mk_page(OBJNAME=None,fix=False):
    ...
    web = open(os.environ['sne'] + '/photoz/index.html' , 'w',0)
    web.write("<head><link type='text/css' rel='stylesheet' href='http://www.slac.stanford.edu/~pkelly/table.css'></head>\
    ...
    for result in results: 
        ...
        from glob import glob
        if glob(rederrs): # int(dtop2['specs']) == 1:
            specs = '<a href=' + dtop2['objname'] + '/CWWSB_capak.list/redshifterrors.html>Spec/Phot<a>'
        else: specs= ''
        ...
        from glob import glob                                                                           
        pick = '*'
        fs_removed = glob( os.environ['sne'] + '/photoz/' + dtop2['objname'] + '/SLRplots/' + pick + '*removed*png')
        fs_no_rejection = glob( os.environ['sne'] + '/photoz/' + dtop2['objname'] + '/SLRplots/' + pick + '*rejection*png')
        file = open(os.environ['sne'] + '/photoz/' + dtop2['objname'] + '/SLRplots/all.html','w')
        file.write('<h1><font color=red>Outliers Removed</font></h1>')
        for f in fs_removed: # + fs_no_rejection:
            file.write( '<br><img src=' + f.split('/')[-1] + '>\n')
            file.write( '<br><a href=' + f.split('/')[-1].replace('.png','.pdf') + '>PDF</a><br>\n')
        file.write('<br><font color=red><h1>No Outliers Removed</h1></font>')

        for f in fs_no_rejection:
            file.write( '<br><img src=' + f.split('/')[-1] + '><br>\n')
        ...
        if num_fs < 0:
            ...
            if False: #len(results) > 0:
                ...
                try:
                    ...
                    command = 'python magnification.py ' + dtop2['objname'] + ' detect=' + lensing_band + ' aptype=aper APER1 '#  + logfile
                    print command 
            ...
            if False: #len(results) > 0:
                ...
                try:
                    ...
                    for lensing_band in filts: #get_lensing_filts(subdir, dtop2['objname']):
                        ...
                        print command
                        if only_type: command += ' only_type'
...
def get_use_info(subdir,cluster):
    ...
    for l in f:
        res = re.split('\s+',l[:-1])
        if res[0] != '':
            if res[0] == cluster: #string.find(res[0],cluster) != -1:  
                use = res[1]
...
def get_lensing_filts(subdir,cluster,get_lensext=False):
    ...
    for l in f:
        ...
        if res[0] != '' and res[0] != '#':
            if cluster == res[0]:  
                #print cluster, res[0]
                if string.find(res[2],'good') != -1: lensext = 'good'
                else: lensext = res[2]
    ...
    while loop:        
        ...
        if False: #len(results) > 0:
            ...
            try:
                ...
                #command = 'do2.sh  ' + dtop2['info'] #+ ' >& ' + logfile
                #os.system(command)
                if float(a) != 0: 
...
def paper_stats():
    ...
    c = db2.cursor()
    db_keys_t = describe_db(c,['' + test + 'try_db'])
    command="SELECT * from " + test + "try_db limit 1" # where todo='good' and var_correction > 0.08 order by rand()"           

    st = [[['None_std','None_mean','None_var_correction']," from test_try_db where todo='good' and None_var_correction is not null",''],
            [['sdss_std','sdss_mean','sdss_var_correction']," from test_try_db where todo='good' and sdss_var_correction is not null",'SDSS']]

    table = ''
    for config, cname in [[' and config like "10%"','10'],[' and (config=8 or config=9)','8~\&~9'],]:
        for list, com, name in st:                                                                
            #command = "select count(*), avg(None_mean) from test_try_db where todo='good'"
            d = 'select count(*), ' + reduce(lambda x,y: x + ', ' + y, ['avg(' + z + ')' for z in list] ) + com + config
            print d
            c.execute(d)                                             
            (count, sdss_std, sdss_mean, sdss_var_correction) =c.fetchall()[0]                                           
            print name, cname 
            print count, sdss_std, sdss_mean, sdss_var_correction

            for l in list:
                d = 'select ' + l + com + config 
                print d
                c.execute(d)                                             
                e = c.fetchall()
                vec = [o[0] for o in e]
            ...
            dict = {'name':name, 'cname':cname, 'count': count, 'sdss_std':sdss_std, 'sdss_mean':(sdss_mean - 1), 'sdss_var_correction':sdss_var_correction}

            table += '%(cname)s&%(name)s&%(count)d&%(sdss_var_correction).1f\\%%&%(sdss_mean).1f\\%%&%(sdss_std).1f\\%%\\\\\n' % dict
...
def paper_stats2():
    ...
    template1 = 'select sdss_var_correction from test_try_db where sdss_var_correction is not null'
    ...
    template2 = 'select None_var_correction from test_try_db where  none_var_correction is not null'
    c.execute(template1)                                             
    e = c.fetchall()
    vec += [o[0] for o in e]
    ...
    pylab.ylabel("Number of Pointings")
...
def construct_correction(OBJNAME,FILTER,PPRUN,sample,sample_size,OBJNAME_use=None,FILTER_use=None,PPRUN_use=None,r_ext=True):
    ...
    try:
        ...
        for ROT in ROTS:
            ...
            import MySQLdb, sys, os, re, time                                                                     
            db2,c = connect_except()
            command  ="select file, ROTATION from illumination_db where SUPA not like '%I' and OBJNAME='" + OBJNAME + "' and PPRUN='" + PPRUN + "'" # and file like '%004007%' " # and ROTATION='" + str(ROT) + "'"
                ...
                for CHIP in CHIPS: 
                    ...
                    if not child:
                        ...
                        if 1:
                            ...
                            BADCCD = False
                            if glob(out_file):
                                command = 'dfits ' + file_chip + ' | fitsort BADCCD'
                                print command
...
def calcDataIllum(file, LENGTH1, LENGTH2, data,magErr, X, Y, pth='/nfs/slac/g/ki/ki04/pkelly/plots/', rot=0, good=None, limits=[-0.4,0.4], ylab='SUBARU-SDSS'):
    ...
    array = {'file':file, 'LENGTH1':LENGTH1, 'LENGTH2': LENGTH2, 'data': data, 'magErr':magErr, 'X':X, 'Y':Y, 'pth':pth, 'rot':rot, 'good':good, 'limits':limits, 'ylab':ylab}
