...
### $Id: do_Subaru_preprocess_2009-01-23_W-J-V.sh,v 1.1 2010-02-04 19:55:00 anja Exp $
...
# Flatfields: DOMEFLATS work very well; variations in the
# illumination pattern are very low (well, it's only one run...)
# SKYFLATs fare somewhat worse - there are star "holes" left,
# and in some instances the different read-out channels can still
# be seen.
# Also note that the SKYFLATS have higher counts (10000 - 30000)
# than the DOMEFLATS (~8000) . The latter then presumably are a
# better representation of typical flux counts... (really???)

# Note: the STANDARD frames show differences for the read-out
# channels (about 1 count in 30 background counts). This could
# be another indication for non-linearity in the CCDs...
