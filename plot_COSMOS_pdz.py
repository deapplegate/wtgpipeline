...
def load_probs():
    import scipy
    P = scipy.loadtxt('/nfs/slac/g/ki/ki05/anja/SUBARU/COSMOS_PHOTOZ/PHOTOMETRY_W-C-IC_' + ext + '/COSMOS_PHOTOZ.APER.1.' + set + '.list.all.probs')
...
def run(P):
    ...
    C = pyfits.open('/nfs/slac/g/ki/ki05/anja/SUBARU/COSMOS_PHOTOZ/PHOTOMETRY_W-C-IC_' + ext + '/cosmos_lephare.cat')['OBJECTS'].data
    print 'loading BPZ'
    U = pyfits.open('/nfs/slac/g/ki/ki05/anja/SUBARU/COSMOS_PHOTOZ/PHOTOMETRY_W-C-IC_' + ext + '/COSMOS_PHOTOZ.APER.1.' + set + '.list.all.bpz.tab')['STDTAB'].data
    print 'loading PDZ'
    print 'done'
    ...
    for low,high in [[0,0.1],[0.1,0.2],[0.2,0.3],[0.3,0.4],[0.4,0.5],[0.5,0.6],[0.6,0.7],[0.7,0.8],[0.8,0.9],[0.9,1.0],[1.0,1.2],[3,4]]:
        ...
        if  True:
            pylab.clf()                                                                                                                    
            out_zs = (U.field('BPZ_Z_B') < high) * (U.field('BPZ_Z_B') > low) * (U.field('BPZ_ODDS') > 0) * (C.field('zp_best') > 0)      
