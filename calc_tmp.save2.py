def sextract(filter):
    ...
    for file in files: #[0:30]:
        if string.find(file,'wcs') == -1 and string.find(file,'.sub.fits') == -1:
            ...
            if not exposures[res[0]]['keywords'].has_key('ROTATION'): #if exposure does not have keywords yet, then get them
                ...
                kws = utilities.get_header_kw(file,['ROTATION','OBJECT']) # return KEY/NA if not SUBARU 
                for kw in kws.keys(): 
    ...
    #exp_tmp = {}
    #for exposure in exposures.keys()[0:2]:
...
def match_simple(exposures,cluster):
    import os
    print exposures
    ...
    from glob import glob
    if len(glob(starcat)) == 0:
        image = exposures[exposures.keys()[0]]['images'][0]
        print image
        command = 'python retrieve_test.py ' + image + ' ' + starcat
        print command    
        os.system(command)
    ...
    for exposure in exposures.keys():
        ...
        print catalog
        outcat = '/tmp/matched.cat'
        command = 'match_simple.sh ' + catalog + ' ' + starcat + ' ' + outcat
        print command
...
if 1:
    ...
    type = 'galaxy'
    exposures = match_simple(exposures,cluster)
