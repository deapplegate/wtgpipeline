...
grid()
f1.savefig('plt_cosmos_ra_dec_compare_ordered')

## 
raoff=(match_2015['ALPHA_J2000']-match_4cc['ra'])*3600
decoff=(match_2015['DELTA_J2000']-match_4cc['dec'])*3600

bins=linspace(-1.0,1.0,40)
f2=figure(figsize=(14,7))
suptitle('position offset between id-matched catalogs from 2015 and 2008')
f2.add_subplot(121)
title('ra/dec offset ')
hist(raoff,bins=bins,histtype='step',label='ra')
hist(decoff,bins=bins,histtype='step',label='dec')
legend()
f2.add_subplot(122)
posoff=sqrt(raoff**2 + decoff**2)
bins=linspace(0,1.1,40)
title('sqrt(ra**2 + dec**2) offset ')
hist(posoff,bins=bins)
tight_layout()
f2.subplots_adjust(top=.93)
f2.savefig('plt_cosmos_ra_dec_pos_offset_hists')
show()
