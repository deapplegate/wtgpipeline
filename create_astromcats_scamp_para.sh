...
for CHIP in $4
do
  ${P_FIND} /$1/$2/ -maxdepth 1 -name \*_${CHIP}[!0-9]\*.fits | awk '{if($1!~"sub.fits" && $1!~"I.fits") print $0}' > ${TEMPDIR}/astromimages_$$
  ...
  cat ${TEMPDIR}/astromimages_$$ |\
  {
	...
	FILTER=`${P_DFITS} ${file} | ${P_FITSORT} FILTER | ${P_GAWK} '($1!="FILE") {print $2}'`
        ...
        ${P_SEX} ${file} -c ${DATACONF}/singleastrom.conf.sex\
  		       -CATALOG_NAME /$1/$2/cat_scamp/${BASE}.cat\
  		       -SEEING_FWHM $fwhm \
