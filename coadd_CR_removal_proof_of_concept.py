...
resamp_fo=astropy.io.fits.open(resamp_chip_fl)
resamp_image=resamp_fo[0].data
resamp_header=resamp_fo[0].header
resamp_fo.close()
COMIN1=resamp_header['COMIN1']
COMIN2=resamp_header['COMIN2']
#del# resamp_chip_slice=(slice(COMIN1,resamp_image.shape[0]+COMIN1,None), slice(COMIN2,resamp_image.shape[1]+COMIN2,None))
resamp_chip_slice=(slice(-1+COMIN2,-1+COMIN2+resamp_image.shape[0]),slice(-1+COMIN1,-1+COMIN1+resamp_image.shape[1]))
supa_coadd_fl='/gpfs/slac/kipac/fs1/u/awright/SUBARU/RXJ2129/W-C-RC/SCIENCE/coadd_RXJ2129_SUPA0135155/coadd.fits'
supa_fitsfl=astropy.io.fits.open(supa_coadd_fl)
supa_image=supa_fitsfl[0].data
supa_header=supa_fitsfl[0].header
supa_fitsfl.close()
chip_selection=supa_image[resamp_chip_slice]
mask=chip_selection==0
resamp_compare=resamp_image.copy()
resamp_compare[mask]=0

print chip_selection.shape, resamp_compare.shape
print (chip_selection == resamp_compare).all()
print (chip_selection-resamp_compare).max()
print (chip_selection-resamp_compare).mean()

n=8;m=n;dm=4
print chip_selection[n:n+dm,n:n+dm]
print resamp_image[m:m+dm,m:m+dm]
print chip_selection[n:n+dm,n:n+dm]/ resamp_image[m:m+dm,m:m+dm]
ratio_image = chip_selection / resamp_image
ratio_image.max()
ratio_image.nanmax()
print nanmax(ratio_image),nanmin(ratio_image[ratio_image>0])

imagetools.PlotLockedImages(chip_selection,resamp_compare*0.0044,'chip-region taken from coadd.fits','resampled chip image (SUPA....resamp.fits)',True)
show()
...
#def mosaic_to_resamp_chip(supa_coadd,position):
#	resamps=['...'] #adam
sys.exit()
