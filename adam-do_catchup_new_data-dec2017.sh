...
if [ "${BeforeSuperflat}" != "DONE" ] ; then
	./process_split_Subaru_eclipse.sh ${SUBARUDIR}/${run}_${filter}SF2/ SCIENCE
	...
	#THIS TAKES ~15 min#
	if [ "$config" == "10_3" ] ; then
		./parallel_manager.sh ./process_science_4channels_eclipse_para.sh ${SUBARUDIR}/${run}_${filter}SF2 BIAS ${FLAT} SCIENCE RESCALE ${FRINGE} #8
	...
	./parallel_manager.sh ./spikefinder_para.sh ${SUBARUDIR}/${run}_${filter}SF2 SCIENCE SUPA OCF ${filter}
	...
	./create_norm_many.sh ${SUBARUDIR}/${run}_${filter}SF2 SCIENCE SUP OCF #9
	exit_stat=$?
	if [ "${exit_stat}" -gt "0" ]; then
		exit ${exit_stat};
	fi
	./create_binnedmosaics.sh ${SUBARUDIR}/${run}_${filter}SF2 SCIENCE_norm SUP "OCFN" 8 -32 #9
	...
	echo "#adam-look# mv ${SUBARUDIR}/${run}_${filter}SF2/SCIENCE/SUPA*OCF.fits ${SUBARUDIR}/${run}_${filter}/SCIENCE for those that will actually help the super-flat"
fi
exit 0;
#questions#
