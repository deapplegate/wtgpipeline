...
#final#run=2010-02-12 ; filter="W-C-IC" ; FLAT=DOMEFLAT
#final#run=2010-02-12 ; filter="W-C-RC" ; FLAT=DOMEFLAT
#final#run=2010-02-12 ; filter="W-J-B" ; FLAT=DOMEFLAT
#final#run=2010-02-12 ; filter="W-J-V" ; FLAT=DOMEFLAT
#final#run=2010-04-15 ; filter="W-S-G+" ; FLAT=SKYFLAT
#final#run=2010-04-15 ; filter="W-S-I+" ; FLAT=DOMEFLAT
#final#run=2011-01-06 ; filter="W-C-IC" ; FLAT=DOMEFLAT
#final#run=2011-01-06 ; filter="W-S-Z+" ; FLAT=DOMEFLAT

#10_2#
#final#run=2006-03-04 ; filter="W-C-RC" ; FLAT=DOMEFLAT
...
###################################################################
./BonnLogger.py clear
#adam# #$#=commenting out this, not needed to finalize the MACS1226 10_3 data
...
#10_2#./parallel_manager.sh ./process_flat_eclipse_para.sh ${SUBARUDIR}/${run}_${filter} BIAS ${FLAT} #6
#$#exit 0; #6
#### creates overview mosaics; use these to identify bad frames
...
#10_2 & 10_3# this is where process_science_4channels_eclipse_para.sh calls things OCF and process_science_eclipse_para.sh calls things OFC, so you gotta change that in this code (tried changing process_science_eclipse_para.sh, but it won't work)
#$#exit 0; #8
...
./create_global_weights_flags_para.sh ${SUBARUDIR}/${run}_${filter} BASE_WEIGHT 0.79 1.11 DARK -1.88 4.9 3
./create_global_weights_flags_para.sh ${SUBARUDIR}/${run}_${filter} BASE_WEIGHT 0.69 1.08 DARK -1.88 5.35 4
./create_global_weights_flags_para.sh ${SUBARUDIR}/${run}_${filter} BASE_WEIGHT 0.32 1.06 DARK -2.94 5.95 5
./create_global_weights_flags_para.sh ${SUBARUDIR}/${run}_${filter} BASE_WEIGHT 0.37 1.07 DARK -1.88 5.5 6
./create_global_weights_flags_para.sh ${SUBARUDIR}/${run}_${filter} BASE_WEIGHT 0.74 1.10 DARK -1.73 4.75 7
