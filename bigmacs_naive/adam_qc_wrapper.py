...
psfPogCorr_z psfPogErr_z SDSS-z.res HOLD 0')
    ...
    if brightSDSS:
        command = 'python fit_locus.py -f ' + catalog + ' -c ' + columnsSDSS + ' -e 1 -b 0 -a -p ' + os.environ['sne'] + '/photoz/' + cluster + '/brightSDSSplots/'

    elif faintSDSS:
        command = 'python fit_locus.py -f ' + catalog + ' -c ' + columnsSDSS + ' -e 1 -b 0 -a -p ' + os.environ['sne'] + '/photoz/' + cluster + '/faintSDSSplots/'

    elif len(filterlist) < 4:
        command = 'python fit_locus.py -f ' + catalog + ' -c ' + columns + ' -e 1 -b 0 -a -p ' + os.environ['sne'] + '/photoz/' + cluster + '/SLRplots/'
        useSDSS = True
    else:
        command = 'python fit_locus.py -f ' + catalog + ' -c ' + columns + ' -e 1 -b 0 -p ' + os.environ['sne'] + '/photoz/' + cluster + '/SLRplots/'
    ...
    if faintSDSS or brightSDSS:
        ...
        #adam-tmp# magtype = 'APER1'
        magtype = 'APER'
        
        if magtype == 'APER1': aptype='aper'
        elif magtype == 'APER': aptype='aper'
	    ...
	    #adam-tmp# command = os.environ['bonn'] + '/save_slr.py --cluster "%(cluster)s" -i %(catalog)s -o %(offset_list)s %(save_slr_flag)s' % {'cluster':cluster, 'catalog':catalog, 'offset_list':offset_list_reformat, 'save_slr_flag':save_slr_flag}
            command = os.environ['bonn'] + '/bigmacs_naive/adam_save_slr.py --cluster "%(cluster)s" -i %(catalog)s -o %(offset_list)s %(save_slr_flag)s' % {'cluster':cluster, 'catalog':catalog, 'offset_list':offset_list_reformat, 'save_slr_flag':save_slr_flag}
            print command
...
if __name__ == '__main__':
    ...
    #./adam_qc_wrapper.py $cluster detect=W-C-RC aptype=aper
