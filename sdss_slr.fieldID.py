...
def sdss_cat(fieldIDs):   

    fieldID_OR = '(' + reduce(lambda x,y: x + ' or ' + y, ['s.fieldID=' + str(x) for x in fieldIDs]) + ')'

    query = 'select ccFlag, t.j as psfmag_J, sqrt(1/t.jivar) as psfmagerr_J, s.psfmag_u, s.psfmag_g, s.psfmag_r, s.psfmag_i, s.psfmag_z, s.psfmagerr_u, s.psfmagerr_g, s.psfmagerr_r, s.psfmagerr_i, s.psfmagerr_z from twomass as t, star as s where t.objID=s.objID and ' + fieldID_OR + ' and t.jivar != 0 and flags & dbo.fPhotoFlags(\'BLENDED\') = 0 and ccFlag = "000" '
