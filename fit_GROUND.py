...
for cluster,clusterz in list:
    if 1: #try:
        ...
        command = 'ldacfilter -i ' + catfile + " -c '(e1_acs > 0.0);' -t PSSC -o " + catfile+'.tmp'
        #os.system('rm ' + catfile + '.tmp')
        print command
        #os.system(command)
        ...
        ran = [[0.3,0.5],[0.65,1.],[1.,1.5]] #,[0.8,1.2],[1.2,1.8],[1.8,3.0]]
        #ran = [[0.5,0.65],[0.65,0.9],[0.9,1.5]] #,[1.8,3.0]]
        ...
        pylab.errorbar(scipy.array(zs_f),scipy.array(amps_f)/infinite,scipy.array(ampErrs_f)/infinite, fmt='k.', markersize=20)
        pylab.xlabel('z',fontsize=16)
        pylab.ylabel('$\gamma$(z)/$\gamma$($\infty$)',fontsize=16)
        pylab.ylim(-0.5,1.6)
        pylab.xlim(0,1.5)
        pylab.axhline(y=0,linestyle='--',c='black')
        
        pylab.savefig(cluster + 'GROUNDratio.ps')
        pylab.show()
