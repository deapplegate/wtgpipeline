...
def select_analyze():
    ...
    while len(results) > 1 or first:
        ...
        command= "SELECT * from fit_db where correction_applied!='yes' and sdss$all$positioncolumns is not null and (sdss$good='y' or nosdss$good='y') ORDER BY RAND()" # and PPRUN='2003-04-04_W-C-IC'"  
...
def match_OBJNAME(OBJNAME=None,FILTER=None,PPRUN=None):
    ...
    while True: 
        ...
        if OBJNAME is None or start == 0:
            ...
            c.execute(command)
            command = "select i.* from temp i left join fit_db f on (i.pprun=f.pprun and i.OBJNAME=f.OBJNAME) where i.PPRUN !='KEY_N/A' and i.file not like '%CALIB%' and i.pprun like '%' and (linearfit!=1 or linearfit is null) and i.OBJNAME not like 'SXDS' and i.OBJNAME not like 'HDFN' and i.pasted_cat is not null and f.linearfit!=1 and i.pprun is not null GROUP BY i.pprun,i.filter,i.OBJNAME ORDER BY RAND() limit 1 " # and PPRUN='2006-12-21_W-J-B' GROUP BY OBJNAME,pprun,filter"
            ...
            try: 
                ...
                if True:
                    ...
                    if len(results) > 0:
                        ...
                        print field 
                        ''' cov is True if there is a 'fake' SDSS catalog or if it is in SDSS coverage '''
                        if true_sdss:
                        ...
                        if cov and true_sdss is True: #d['starcat'] == 'None':
                        ...
                        linear_fit(OBJNAME,FILTER,PPRUN,cov,CONFIG,true_sdss,primary=primary,secondary=secondary)
                        if true_sdss is False and cov is True:
                            save_fit({'piggyback':'yes','PPRUN':PPRUN,'OBJNAME':OBJNAME,'FILTER':FILTER})
...
def sort_results(results2,dict,db_keys):
    ...
    for line in results2: 
        ...
        if float(dict['EXPTIME']) > 10.0:
            if not dict['PPRUN'] in rotation_runs:                                                                                                                               
                rotation_runs[dict['PPRUN']] = {'ROTATION':{dict['ROTATION']:'yes'},'FILTER':dict['filter'],'CONFIG_IM':CONFIG_IM,'EXPTIME':dict['EXPTIME'],'file':dict['file'],'linearfit':dict['linearfit'],'OBJNAME':dict['OBJNAME'],'catalog':dict['catalog'],'FILTER_NUM':FILTER_NUM,'sdss$good':dict['sdss$good'],'nosdss$good':dict['nosdss$good']}
            rotation_runs[dict['PPRUN']]['ROTATION'][dict['ROTATION']] = 'yes'
    print rotation_runs
...
def selectGoodStars(EXPS,cov,LENGTH1,LENGTH2,relative_colors):
    ''' the top two most star-like objects have CLASS_STAR>0.9 and, for each rotation, their magnitudes differ by less than 0.01 '''
    import pyfits, sys, os, re, string, copy , string, scipy
            ...
            if len(list) > 1:
                ...
                if len(file_list) > 1:
                    ...
                    if relative_colors:
                        if sdss==1:    
                            star_good.append(i)                                                                                                     
                            supas.append({'mag':mag,'table index':i,'supa files':file_list, 'sdss':sdss, 'sdss_exists':sdss_exists})                        
                    else:
                        star_good.append(i)                                                                                                     
                        supas.append({'mag':mag,'table index':i,'supa files':file_list, 'sdss':sdss, 'sdss_exists':sdss_exists})                        
        if i%2000==0: print i
...
def linear_fit(OBJNAME,FILTER,PPRUN,cov=None,CONFIG=None,true_sdss=None,primary=None,secondary=None):

    print cov, CONFIG, true_sdss
    ...
    ''' if comparison is not SDSS and early chip configuration, use chip color terms ''' 
    if not true_sdss and (CONFIG=='8' or CONFIG=='9'):
    ...
    if not quick:
        EXPS, star_good,supas, totalstars = selectGoodStars(EXPS,cov,LENGTH1,LENGTH2,relative_colors)               
    ...
    print "calc_test_save.linear_fit('" + OBJNAME + "','" + FILTER + "','" + PPRUN + "'," + str(cov) + ",'" + CONFIG + "'," + str(true_sdss) + ",primary='" + str(primary) + "',secondary='" + str(secondary) + "')"
    print len(star_good)
    ...
    for sample,which_terms,cov in samples:
        ...
        save_fit({'PPRUN':PPRUN,'FILTER':FILTER,'OBJNAME':OBJNAME,sample + '$primary_filt':primary,sample + '$secondary_filt':secondary,'sdss_coverage':str(cov),sample + '$relative_colors':relative_colors,sample + '$true_sdss':true_sdss,'CONFIG':CONFIG})
        ...
        for sample_size in ['all']: #'rand1','rand1']: #,'rand2','rand3','rand4','all']: #,'rand3']:
            ...
            if cov: 
                ...
                index += 1
                color_columns+=[{'name':'SDSS_color','image':'sdss_color_term','index':index, 'chip_group':[]}]


            else: color_columns = []
            columns += color_columns
            print color_columns, cov, 
