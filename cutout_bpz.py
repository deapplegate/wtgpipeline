...
def make_thecorrections(cluster,DETECT_FILTER,SPECTRA='CWWSB_capak.list',AP_TYPE='_aper',magtype='APER1',type='all'):
    ...
    f2 = datetime.now()
    pagemain.write('created ' + f2.strftime("%Y-%m-%d %H:%M:%S") + '<br>\n')
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
    pagemain.write('<br>CORRECTIONS<br><ul>')
    ims = ''
    nums = '<br><br>NUMBER OF GOOD STARS<br><ul>\n'
    for key in keys2:
        if key != 'ID' and key != 'Z_S' and stars_dict.has_key(key):
...
def convert_probs_to_fits_ldac(file):
    ...
    for i in range(len(zs)):
        cols.append(str(zs[i]))

    tempconf = '/tmp/' + os.environ['USER'] + 'probs.conf'
    ...
    command = 'asctoldac -i ' + file + ' -c ' + tempconf + ' -o ' + file + '.tab -t STDTAB'
    print command
    os.system(command)
    raw_input('finished')
...
def convert_probs_to_fits(file):
    ...
    go = True
    if glob(file + '.tab'):
        if os.stat(file + '.tab') > os.stat(file): go = False
        
        

    if go:

        print 'converting ascii file to fits file' 
        p = open(file).readlines()[0]                                               
    ...
    for filt in list:
        for f2 in filt:
            ...
            if ok:                                                       
                ...
                instrument_to_locus[f2] = a_short

    print locus_ddict, instrument_to_locus
                        ...
                        for c in [locus_ddict[f2A][random.randint(0,len(locus_ddict[f2A])-1)]]:
                            for d in [locus_ddict[f2B][random.randint(0,len(locus_ddict[f2B])-1)]]:
                                ...
                                if len(at) and len(bt) and len(ct) and len(dt) and len(locus_c[f1A + '_' + f1B])==len(locus_c[f2A + '_' + f2B]):                   
                                    ...
                                    pylab.errorbar(x,y,xerr=xerr,yerr=yerr,fmt='ro',c='blue')    
                                    ...
                                    pylab.errorbar(locus_c_old[f1A + '_' + f1B],locus_c_old[f2A + '_' + f2B],c='red',fmt='ro')                  
...
def plot_bpz_probs(ids,file,outfile,zspec=None):    
    import re, scipy
    f = pyfits.open(file)[1]

    x_axis = []
    for col in f.columns[1:]:
        x_axis.append(float(col.name))

    x_axis_all = scipy.array(x_axis)
    ...
    for i in range(len(ids)):
        ...
        id = ids[i]
        print 'making PDZ plot for ', id, 'id'
        mask = f.data.field('SeqNr') == id
        print f.data[mask]
        print f.data.field('SeqNr')[mask]
        print f.data.field('SeqNr')
        print mask
        arr = f.data[mask][0]
        ...
        #print arr, len(arr), 'length arr'
        arr = scipy.array(arr )
        mask = arr>0
        x_axis = copy(x_axis_all)
        x_axis = x_axis[mask]
        ...
        #pylab.plot(x_axis, arr,c='black',lw=2)
        pylab.bar(x_axis,arr,align='center',width=0.01,facecolor='none',edgecolor='black') #,color='none',edgecolor='black')

        if zspec is not None:
            z = zspec[i]
            pylab.axvline(z,c='red',linewidth=2.0)
        
        pylab.xlabel("Redshift")
        pylab.ylabel("Probability")
        fout = outfile + str(id) + '.png'
        print fout
        pylab.savefig(fout)

        fout = outfile + str(id) + '.pdf'
        print fout
        pylab.savefig(fout)
...
def run(cluster):
    ...
    output_dir = os.environ['sne'] + '/photoz/'
    ...
    if len(sys.argv) > 2:
        for s in sys.argv:
            ...
            if string.find(s,'list') != -1:
                ...
                rs = re.split('=',s)
                LIST=rs[1]
            if string.find(s,'seqnrs') != -1:
                import re
                rs = re.split('=',s)
                SEQNRS=rs[1]
            if string.find(s,'output') != -1:
                import re
                rs = re.split('=',s)
                output_dir=rs[1]
    ...
    print 'OUTPUT CATALOG', outputcat
    ...
    os.system('mkdir -p ' + output_dir + cluster + '/' + SPECTRA + '/') 
    ...
    print 'PHOTOMETRY CATALOG:', catalog

    
    outbase = output_dir + cluster + '/'
    os.system('mkdir -p ' + outbase + '/' + SPECTRA + '/')
    ...
    if type == 'spec':
        outputcat = '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.%(magtype)s.%(SPECTRA)s.1.%(type)s.bpz.tab' % params  
    ...
    ffile = output_dir + cluster + '/fix.tif'

    if not glob(ffile):

        #print filters[1:4]
        if len(filters) > 3:
            filts_use = filters[1:4]
        else: filts_use = filters[:]
        for filt in filts_use:                                                                 
            params_stiff = {'path':path, 
                      'filter_root': filter_root, 
                      'cluster':cluster, 
                      'filter':filt,
                      'DETECT_FILTER': DETECT_FILTER,  
            ...
            image = '%(path)s/%(filter)s/SCIENCE/coadd_%(cluster)s%(appendix)s/coadd.fits' %params_stiff 
            images.append(image)
            print 'reading in ' + image        
            seg_image = '/%(path)s/%(filter)s/PHOTOMETRY/coadd.apertures.fits' % params_stiff
        ...
        os.system('mkdir ' + output_dir + cluster + '/')
        os.system('chmod o+rx ' + output_dir + cluster + '/')
        
        from glob import glob

	images.reverse() # we want RED GREEN BLUE order
	print images

    if remakeImage:
        os.system('stiff ' + reduce(lambda x,y:x + ' ' + y,images) + ' -OUTFILE_NAME ' + ffile + ' -BINNING 1 -GAMMA_FAC 1.6')
        os.system('convert ' + output_dir + cluster + '/fix.tif ' + output_dir + cluster + '/fix.jpg')
    
    print ffile
   
    import PIL 
    from PIL import Image
    im = Image.open(output_dir + cluster + '/fix.jpg')
    ...
    if type == 'spec':
        probsout = '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.%(magtype)s.%(SPECTRA)s.1.%(type)s.probs' % params  
    ...
    if False:
        ...
        redseq =  '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.redseq' % params 
        import scipy
        set = [int(e[:-1]) for e in open(redseq,'r').readlines()][0:2]
                                                                                                
        print set


    if not SEQNRS:
        specs =  '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/spec_match_id.list' % params
        array = scipy.loadtxt(specs)[:]               
        mask = array[:,2] > 0.0   
        array = array[mask]
        print array
                                                      
        mask = (array[:,1] > 1.) * (array[:,1] < 1.1)
        array = array[mask]
        set = [int(a) for a in array[:,0]] 
        zspec = [a for a in array[:,1]]
    else:
        array = scipy.loadtxt(SEQNRS)[:]               
        zspec = [-99 for a in range(len(array))]
    ...
    file = '/nfs/slac/g/ki/ki04/pkelly/photoz/' + cluster + '/' +  SPECTRA + '/spec' 
    probs =  '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.%(magtype)s.1.%(SPECTRA)s.%(type)s.probs' % params
    flux_comparison =  '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.%(magtype)s.1.%(SPECTRA)s.%(type)s.flux_comparison' % params

    flux_comparison_nohash =  '%(path)s/PHOTOMETRY%(DETECT_FILTER)s%(AP_TYPE)s/%(cluster)s.%(magtype)s.1.%(SPECTRA)s.%(type)s.flux_comparison.nohash' % params

    ''' SED plot cannot handle ##, so remove ''' 
    import os
    command = 'grep -v "#" ' + flux_comparison + ' > ' + flux_comparison_nohash
    print command
    os.system(command)

    command = 'grep -v "#" ' + base + '.bpz > ' + base + '.bpz.nohash'
    print command
    os.system(command)

    command = 'grep "#" ' + base + '.bpz > ' + base + '.bpz.hash'
    print command
    os.system(command)

    command = 'cat ' + base + '.bpz.hash ' + base + '.bpz.nohash > ' + base + '.bpz'
    print command
    os.system(command)
    ...
    if True:
        plot_bpz_probs(set, probs + '.tab',file + '_probs',zspec=zspec)

    print base, 'base'
    print flux_comparison_nohash

    print ' read in spectrum information '
    filt_infos = sedplot.run_pat(base,set,file,flux_comparison_nohash)
    print set
    print filt_infos.keys()
    ...
    for i in range(len(set)):
        ...
        SeqNr = set[i]
        zs = zspec[i]
#    for SeqNr in set: #[0:25]: #[75227,45311,53658, 52685, 64726]:
        filt_info= filt_infos[SeqNr]
        print SeqNr 
        line = bpz_cat[bpz_cat.field('SeqNr')==SeqNr][0]
        ...
        if True: #line['BPZ_ODDS'] > 0.5 and 0.6 < line['BPZ_Z_B'] < 0.8 and 0.2 < line['BPZ_Z_S'] < 0.4: #abs(resid) > 0.2: #abs(resid) > 0.04 and 0.4 < line['BPZ_Z_S'] < 0.6: #True: #SeqNr==24778: # SeqNr == 4441 or SeqNr==1285: #abs(resid) > 0.1:    
            ...
            if zs: #line['BPZ_Z_S'] != 0: 
                resid = line['BPZ_Z_B'] - zs #line['BPZ_Z_S']
                if resid > 0: 
                    color='green'
            ...
            text += '<td align=left><img height=400px src=spec_probs' + str(index)  + '.png></img>\n'
            text += '<img height=400px src=spec' + str(index)  + '.png></img>\n'
            images = []
            ...
            if 1: #try:
                ...
                image = output_dir + cluster + '/' + SPECTRA + '/' + str(index) + '.jpg'  
                print image
                os.system('rm ' + image)
                p.save(image,'JPEG')
            #except: print 'fail'
            ...
            from datetime import datetime
            t2 = datetime.now()
            file = output_dir + cluster + '/' + SPECTRA + '/objects.html'
            print file
            page = open(file,'w')
