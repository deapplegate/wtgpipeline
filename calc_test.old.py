            ...
            if not child:
                if (search_params['CORRECTED']!='True' or (search_params['CORRECTED']=='True' and string.find(str(search_params['CRVAL1ASTROMETRY_' + NUM]),'None') == -1)):
                    try:                                                                                                                                                                                                                                                                                                                                                                                                               
                        ...
                        SDSS2 = "/%(path)s/%(fil_directory)s/SCIENCE/headers_scamp_SDSS-R6/%(BASE)sO*.head" % params # not a zero!!!!!
                        from glob import glob 
...
def match_OBJNAME(SDSS=False):
    ...
    command="SELECT * from illumination_db where file not like '%CALIB%' and SUPA not like '%I' and OBJNAME like 'MACS0850%' and filter='W-S-Z+' and PPRUN !='KEY_N/A' GROUP BY OBJNAME,pprun,filter" # and PPRUN='2006-12-21_W-J-B' GROUP BY OBJNAME,pprun,filter"
    print command
    c.execute(command)
    results=c.fetchall()
    for line in results: 
    ...
    for sample,cheby_terms_use in samples:
        ...
        for sample_size in ['rand1']: #'rand1','rand2','rand3','rand4']: #,'rand3']:
            ...
            if sample_size != 'all':     
                ...
                supas = [supas_copy[i] for i in l[0:len(supas_copy)/12]]
            else:
