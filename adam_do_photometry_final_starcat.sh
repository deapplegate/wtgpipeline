...
if [ -z "${lensing_coadd_type}" ]; then

	#lensing_coadd_type="good" #adam: default is to use "good" coadd for lensing
	echo "adam-Error: you need to include the lensing coadd-type for ${cluster} in lensing_coadd_type_filter.list!"
	exit 1
fi
...
if [ $fit_calibration -eq 1 ]; then
	...
	#adam-important#
	# usually BigMacs is run on      :    ${photdir}/${cluster}.stars.cat that was created prior to the ldaclensing scripts, but not anymore!
	# now use ldaclensing cat instead:    ...which goes by the same name...you just have to re-run check_psf_coadd_vis_new.sh once more after selecting the polynomial order.
	python adam_bigmacs-cat_array_splitter.py -i ${star_cat} -o ${photdir}/${cluster}.stars.split_apers.cat
	exit_code=$?
	...
	export PYTHONPATH
	cd $BIGMACS
	mkdir ${photdir}/BIGMACS_output_PureStarCalib/
...
if [ $apply_calibrations -eq 1 ]; then
	...
	if [ ! -s "${photdir}/${cluster}.calibrated_PureStarCalib.cat" ]; then
