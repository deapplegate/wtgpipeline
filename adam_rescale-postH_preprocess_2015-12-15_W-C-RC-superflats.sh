...
filter=${pprun#2*_}
run=${pprun%_*}
SET=SET2            # sets time period of flat to use
...
SCIENCEDIR=SCIENCE_${FLAT}_${SET}_rescale
...
./parallel_manager.sh ./create_illumfringe_stars_para.sh ${SUBARUDIR}/${run}_${filter} $SCIENCEDIR ${SKYBACK} > OUT-create_illumfringe_stars_para-${SCIENCEDIR}.log 2>&1
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
...
./create_norm.sh ${SUBARUDIR}/${run}_${filter} ${SCIENCEDIR} > OUT-create_norm-${SCIENCEDIR}.log 2>&1
\ls -lrth ${SUBARUDIR}/${run}_${filter}/${SCIENCEDIR}_norm > FILES-create_norm-${SCIENCEDIR}.log
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
...
./create_norm_illum_fringe.sh ${SUBARUDIR}/${run}_${filter} ${SCIENCEDIR} $SKYBACK > OUT-create_norm_illum_fringe-${SCIENCEDIR}.log 2>&1
\ls -lrth ${SUBARUDIR}/${run}_${filter}/${SCIENCEDIR}_norm > FILES-create_norm_illum_fringe-${SCIENCEDIR}.log
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
...
./make_residuals.sh ${SUBARUDIR}/${run}_${filter} $SCIENCEDIR ${SKYBACK} > OUT-make_residuals-${SCIENCEDIR}.log 2>&1
\ls -lrth ${SUBARUDIR}/${run}_${filter}/${SCIENCEDIR}_norm > FILES-make_residuals-${SCIENCEDIR}.log
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
...
#tmp#./make_binned_sets.sh ${SUBARUDIR}/${run}_${filter}/SCIENCE_norm/BINNED > OUT-make_binned_sets-${SCIENCEDIR}.log 2>&1
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
...
if [ ${FRINGE} == "NOFRINGE" ]; then
  ...
  ./parallel_manager.sh ./process_science_illum_eclipse_para.sh ${SUBARUDIR}/${run}_${filter} ${SCIENCEDIR}_norm RESCALE ILLUM ${SKYBACK} ${SCIENCEDIR} > OUT-process_science_illum_eclipse_para-${SCIENCEDIR}.log 2>&1
else
  ./parallel_manager.sh ./process_science_illum_fringe_eclipse_para.sh ${SUBARUDIR}/${run}_${filter} ${SCIENCEDIR}_norm RESCALE ${SKYBACK} ${SCIENCEDIR} > OUT-process_science_illum_fringe_eclipse_para-${SCIENCEDIR}.log 2>&1
fi
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
...
./create_binnedmosaics.sh ${SUBARUDIR}/${run}_${filter} ${SCIENCEDIR} SUP ${ending} 8 -32 > OUT-create_binnedmosaics-${SCIENCEDIR}.log 2>&1
exit_stat=$?
if [ "${exit_stat}" -gt "0" ]; then
	exit ${exit_stat};
fi
