...
for filter in filters:
    ...
    run('ldacfilter -i ' + base + file + ' -o /tmp/good.stars -t PSSC\
          -c "(((SEx_IMAFLAGS_ISO=0 AND SEx_CLASS_STAR>0.5) AND (SEx_Flag=0)) AND Flag=0);"',['/tmp/good.stars'])
   ...
   #             -c "(SEx_CLASS_STAR>0.00);"',['/tmp/good.stars'])
    ...
    run('ldaccalc -i /tmp/good.colors -t PSSC -c "(' + dict['filter'] + 'mag - SEx_' + mag + ');"  -k FLOAT -n magdiff "" -o /tmp/all.diff.cat',['/tmp/all.diff.cat'] )

    run('ldactoasc -b -q -i /tmp/all.diff.cat -t PSSC -k SEx_' + mag + ' ' + dict['filter'] + 'mag SEx_FLUX_RADIUS SEx_CLASS_STAR ' + dict['filter'] + 'err ' + dict['color1'] + ' > /tmp/mk_sat',['/tmp/mk_sat'] )
...
#    run('ldactoasc -b -q -i ' + base + file + ' -t PSSC -k SEx_' + mag + '_' + filter +  ' ' + dict['filter'] + 'mag SEx_FLUX_RADIUS SEx_CLASS_STAR ' + dict['filter'] + 'err ' + dict['color1'] + ' > /tmp/mk_sat',['/tmp/mk_sat'] )
    ...
    run('ldactoasc -b -q -i /tmp/filt.mag.cat -t PSSC -k SEx_Xpos_ABS SEx_Ypos_ABS > /tmp/positions',['/tmp/positions'] )

    run('ldacaddkey -i /tmp/filt.mag.cat -o /tmp/filt.airmass.cat -t PSSC -k AIRMASS 0.0 FLOAT "" ',['/tmp/filt.airmass.cat']  )
    
    run('ldactoasc -b -q -i /tmp/filt.airmass.cat -t PSSC -k SEx_' + mag + ' ' + dict['filter'] + 'mag ' + dict['color1'] + ' ' + dict['color2'] + ' AIRMASS SEx_' + magerr + ' ' + dict['filter'] + 'err SEx_Xpos_ABS SEx_Ypos_ABS > /tmp/input.asc',['/tmp/input.asc'] )
