...
def cutout(infile,mag,color='red'):
    '''Makes ds9 region file putting circles around all objects in a mag, magdiff range'''
    import os, utilities
    ppid = str(os.getppid())
...
def get_median(cat,key):
    ...
    magdiff.sort()   #could just call numpy.median
...
def combine_cats(cats,outfile,search_params):
    '''I think this combines catalogs of different filters, appending the filter name, and converting MAG_APER from a vector quantity'''
...
def paste_cats(cats,outfile): #cats,outfile,search_params):
    '''Append catalogs one after the other?'''
...
def imstats(SUPA,FLAT_TYPE):
    '''Save median, sigma info from a bunch of files to the database'''
    import os, re, utilities, bashreader, sys, string
    from copy import copy
    from glob import glob
    dict = get_files(SUPA,FLAT_TYPE)   #database call
    search_params = initialize(dict['filter'],dict['cluster']) #get workdir info
    ...
    save_exposure(dict,SUPA,FLAT_TYPE)  #database call

##########################################################

#THIS FUNCTION OVERRIDDEN BELOW!!!!
def save_fit(fits,im_type,type,SUPA,FLAT_TYPE):
    '''Saves user and time stamp of photometry fit to database'''
...
def select_analyze():
    '''Hardcoded SQL call to collect arbitraty info from the DB, calls analyze'''
    import MySQLdb, sys, os, re, time 
    from copy import copy
    db2 = MySQLdb.connect(db='subaru', user='weaklensing', passwd='darkmatter', host='ki-sr01')
    c = db2.cursor()
    ...
    command = "SELECT * from illumination_db where color1_star > 0.2 and cluster!='HDFN' limit 2" # where matched_cat_star is null" # where cluster='HDFN' and filter='W-J-V' and ROTATION=0"
    command= "SELECT * from illumination_db where zp_star_ is null and cluster='MACS0911+17' and PPRUN='2003-04-04_W-C-IC'"
...
def analyze(SUPA,FLAT_TYPE):
    '''Run analysis. Actual steps selected by hardcoded comment symbols'''
    #try:
    import sys
    import os                   
...
def get_files(SUPA,FLAT_TYPE=None):    
    '''Reads from DB a glob string to execute to find file names. Returns dictionary with entry "files", containing glob output'''
...
def get_fits(CLUSTER,FILTER,PPRUN):
    '''Retuns entries from the fit_db table in a dictionary'''
    import MySQLdb, sys, os, re                                                                     
    db2 = MySQLdb.connect(db='subaru', user='weaklensing', passwd='darkmatter', host='ki-sr01')
    c = db2.cursor()
...
def save_exposure(dict,SUPA=None,FLAT_TYPE=None):
    '''Construct illumination_db table in DB if not exists, and add exposure to it'''
    ...
    floatvars = {}  
    stringvars = {}
    #copy array but exclude lists; det object type
    import string
...
def initialize(filter,cluster):
    '''Read progs.ini, and set some path info'''
...
def update_dict(SUPA,FLAT_TYPE):    
    '''Save info from an exposure header to database'''
    import utilities
...
def gather_exposures(cluster,filters=None):
    '''Find all exposures in a cluster that are calibration exposures, save to DB by flat type'''
    if not filters:
...
def find_seeing(SUPA,FLAT_TYPE):
    '''Run SE on some images, determine seeing'''
...
def length(SUPA,FLAT_TYPE):
    '''determine the total dimensions for an exposure, over all chips'''
    import os, re, utilities, bashreader, sys, string
    from copy import copy
    from glob import glob
    dict = get_files(SUPA,FLAT_TYPE)
    ...
    for image in search_params['files']:
        ...
        #print im
        crpix = utilities.get_header_kw(image,['CRPIX1','CRPIX2','NAXIS1','NAXIS2','CRVAL1','CRVAL2'])
        if start == 1:
    ...
    save_exposure({'crfixed':'third','LENGTH1':LENGTH1,'LENGTH2':LENGTH2,'CRPIX1ZERO':crpixzero['CRPIX1'],'CRPIX2ZERO':crpixzero['CRPIX2'],'CRVAL1':crpix['CRVAL1'],'CRVAL2':crpix['CRVAL2']},SUPA,FLAT_TYPE)

#######################################################################################

def sdss_coverage(SUPA,FLAT_TYPE):
    '''Query SDSS database for overlap coverage of an exposure'''
    ...
    import sqlcl    #AH HA!!! This is an SDSS database tool!
...
def sextract(SUPA,FLAT_TYPE):
    '''First, run astrometry.net, then source extract files matching supa, flat_type'''
    import os, re, utilities, bashreader, sys, string
    from copy import copy
    from glob import glob
            ...
            if 1:
                if 1:
                    ...
                    if 1: #head is None:
                        ...
                        utilities.run(command)
                        os.system('rm ' + imfix)
                        command = 'solve-field --cpulimit 240  --no-verify --no-plots --overwrite --scale-units arcsecperpix --scale-low ' + str(float(params['PIXSCALE'])-0.005) + ' --scale-high '  + str(float(params['PIXSCALE'])+0.005) + ' -N ' + imfix + ' ' + imtmp
                        print command
                        os.system(command)
                        os.system('rm ' + imtmp)
                        from glob import glob
                        if len(glob(imfix)):
                            ...
                            for name in ['CRVAL1','CRVAL2','CD1_1','CD1_2','CD2_1','CD2_2','CRPIX1','CRPIX2']:
                                print name + 'ASTROMETRY', hdict[name]
                                wcsdict[name + 'ASTROMETRY'] = hdict[name]                                        
...
def get_sdss_obj(SUPA, FLAT_TYPE):
    '''looks for a star catalog overlapping image. If not found, then calls module retrieve_test.py to get sdss objects'''
    dict = get_files(SUPA,FLAT_TYPE)
    search_params = initialize(dict['filter'],dict['cluster'])
    search_params.update(dict)
...
def match_simple(SUPA,FLAT_TYPE):
    '''like get_sdss_obj, looks for catalogs of sdss objects; if not found, calls retrieve_test; then calls match_simple.sh'''
...
def phot(SUPA,FLAT_TYPE): 
    '''Does photometry on some files; first it does catalog filtering; then makes saturation plots via mk_saturation_plot.
          Finally, calls photo_abs_new.py to do photometry'''
    dict = get_files(SUPA,FLAT_TYPE)
    print dict.keys()
    search_params = initialize(dict['filter'],dict['cluster'])
    search_params.update(dict)
...
def nightrun():
    '''For a night/run, calls photo_abs_new.calcIllumData'''
...
def auto_print():
    '''Another method to pass files to photo_abs_new.calcDataIllum?'''
    import MySQLdb, sys, os, re, time, utilities, pyfits
    from copy import copy
    db2 = MySQLdb.connect(db='subaru', user='weaklensing', passwd='darkmatter', host='ki-sr01')
    c = db2.cursor()
...
def describe_db(c,db='illumination_db'):
    '''Return name of columns from a DB table'''
    command = "DESCRIBE " + db
    print command
    c.execute(command)
    results = c.fetchall()
...
def printer():
    '''Yet another method that calls photo_abs_new.calcDataIllum. This one with hardcoding of cluster/filter'''
    import MySQLdb, sys, os, re, time, utilities, pyfits
    from copy import copy
    db2 = MySQLdb.connect(db='subaru', user='weaklensing', passwd='darkmatter', host='ki-sr01')
    c = db2.cursor()
...
####ENTRY POINT? Script to run photometry code (find seeing, sextract, sdss matching, then photometry)
                # Is this even used?
import pickle
...
if 0:
    ...
    if 1: 
        images = gather_exposures(filter,cluster)
    
    print images
    
    ''' strip down exposure list '''
    for key in exposures.keys():
        print exposures[key]['images']  #exposures has a dictionary interface
    
        for image in exposures:   #what defines exposures??? They are loaded from a file, saved to a file, but never created?
            if 1: 
                image.find_seeing(exposures) # image is an object?
            if 1: 
                image.sextract(exposures)
            if 1: 
                image.match_simple(exposures,cluster)
            if 1: 
                image.phot(exposures,filter,type,LENGTH1,LENGTH2)
...
def get_sdss(dict):
    '''Another function like get_sdss_obj, looks for star, galaxy cats and calls retrieve_test to get them'''
    import MySQLdb, sys, os, re, time, utilities, pyfits
    from copy import copy
    import os
    search_params = initialize(dict['filter'],dict['cluster'])
    ...
    starcat ='/nfs/slac/g/ki/ki05/anja/SUBARU/%(cluster)s/PHOTOMETRY/sdssstar%(ROTATION)s.cat' % {'ROTATION':search_params['ROTATION'],'cluster':search_params['cluster']}
    galaxycat ='/nfs/slac/g/ki/ki05/anja/SUBARU/%(cluster)s/PHOTOMETRY/sdssgalaxy%(ROTATION)s.cat' % {'ROTATION':search_params['ROTATION'],'cluster':search_params['cluster']}
    path='/nfs/slac/g/ki/ki05/anja/SUBARU/%(cluster)s/' % {'cluster':search_params['cluster']}
    illum_path='/nfs/slac/g/ki/ki05/anja/SUBARU/ILLUMINATION/' % {'cluster':search_params['cluster']}
    #os.system('mkdir -p ' + path + 'PHOTOMETRY/ILLUMINATION/') 
    os.system('mkdir -p ' + path + 'PHOTOMETRY/ILLUMINATION/STAR/') 
...
def match_cluster(SDSS=False):
    '''Hardcoded for cluster, analysis; if SDSS overlap, match, apply photometry, and do "linear fit"'''
    ...
    list = reduce(lambda x,y: x + ',' + y, keystop)
    command="SELECT * from illumination_db where zp_star_ is not null and PPRUN='2002-06-04_W-J-V' and OBJECT='MACSJ1423.8' GROUP BY cluster,ROTATION"
    #command="SELECT * from illumination_db where cluster like '%2243%' and filter='W-J-V' GROUP BY cluster,pprun,filter "
    #command="SELECT * from illumination_db where file not like '%CALIB%' and OBJECT like '%1423%' GROUP BY cluster,pprun,filter"

    command="SELECT * from illumination_db where file not like '%CALIB%' GROUP BY cluster,pprun,filter"
    print command
    c.execute(command)
    results=c.fetchall()
    for line in results: 
        try: 
            ...
            keys = ['SUPA','cluster','ROTATION','PPRUN','pasted_cat','filter','ROTATION','files']
            list = reduce(lambda x,y: x + ',' + y, keys)
            #command="SELECT * from illumination_db where zp_star_ is not null and cluster='"+dtop['cluster'] + "' and PPRUN='" + dtop['PPRUN'] + "'"#+ "' GROUP BY cluster,ROTATION"

            command="SELECT * from illumination_db where CLUSTER='" + dtop['cluster'] + "' and PPRUN='" + dtop['PPRUN'] + "'"#+ "' GROUP BY cluster,ROTATION"
            print command
            c.execute(command)
            results=c.fetchall()
            print results
            ...
            for line in results: 
                ...
                info.append([d['ROTATION'],d['SUPA'],d['cluster']])
            if 0:
...
def apply_photometric_calibration(SUPA,FLAT_TYPE):
    ...
    for key in dict.keys():
        ...
        if string.find(key,'color') != -1:
            print key
    #calib = get_calibrations_threesecond(dict['cluster'],filters_info)
    #print 'calib', calib
    #raw_input()
