...
def makePlotsNew(data, airmass, color1, color2, outfile, fits, label):
    ...
    for i in range(len(fits)):
        ...
        colMinVal = Numeric.sort(color1)[5]
        if colMinVal < 0:
            colMin = colMinVal*1.1
        else:
            colMin = colMinVal*0.95
        colMaxVal = Numeric.sort(color1)[-1]*1.1
        ...
        xaxis_var = fits[i]['plots'][0]['xaxis_var']
...
if __name__ == "__main__":
    ...
    fits = [{'model':[{'name':'zp','term':['zp'],'value':pdict['zp']},{'name':'color1','term':['color1'],'value':pdict['color1']},{'name':'color2','term':['color2'],'value':pdict['color2']}], \
            'fixed':[], \
