...
__cvs_id__ = "$Id: photo_abs_m.py,v 1.7 2010-08-10 21:29:42 anja Exp $"
...
def makeFitLabel(fit):

    jj = [] 
    for term in fit['model']:   
        jj.append(reduce(lambda x,y: x  + 'T' + y,term['term']))
    
    model =  reduce(lambda z,w: z + 'P' + w, jj)
    
    fit['class'].fitvars['model_name'] = model
    
    jj = [] 
    for term in fit['fixed']:   
        jj.append(reduce(lambda x,y: x  + 'T' + y,term['term']))
    if len(jj) > 0: 
        model =  reduce(lambda z,w: z + 'P' + w, jj)
    else: 
        model = ''
    fit['class'].fitvars['fixed_name'] = model


####################################

def run_main(cluster,infile,output,extcoeff,sigmareject,step,bandcomp,color1which,color2which,nonint=0,label=None, fitID=None):
    #############################
    # ARGUMENT DESCRIPTIONS
    #
    # infile : file wtih std mags
    # sigmareject : level of sigma rejection in fit
    # nonint : integer based boolean - controls interactivity
    # label : used in labeling plots

    # Variables that are just info:
    #    cluster, step, bandcomp, color1which, color2which, fitID

    # not used: output, extcoeff
    ##############################
    
    print "nonin = "+str(nonint)
    print infile
    data, airmass, color1, color2, magErr, X, Y, stdMag = readInput(infile)  

    print color1

    fit_param_vals = {'zp':0,'extcoeff':extcoeff,'color1':0,'color2':0}
 
    #write input quantities to dictionary 
    var_names = {'bandcomp':bandcomp, 'color1which':color1which , 'color2which':color2which, 'sigmareject':sigmareject, 'cluster':cluster, 'label':label, 'fitID':fitID}

    fits = [{'model':[{'name':'zp','term':['zp'],'value':fit_param_vals['zp']},{'name':'color1','term':['color1'],'value':fit_param_vals['color1']}], \
                'fixed':[], \
                'apply': ['zp','color1'], \
    ...
    makePlotsNew(data, airmass, color1, color2, X, Y, outfile, fits, label,nonint)
...
def makePlotsNew(data, airmass, color1, color2, X, Y, outfile, fits, label,nonint):

    print "in makePlotsNew ",str(label)

    file = outfile+label+".ps"
    if nonint==1:
        print "in nonint printing to "+ file
        pgbeg(file+"/ps", 1, 1) #2,3
        print "done"
    else:
        print "in int"
        pgbeg("/XWINDOW",1,1)
...
def saveResults(file, fits, var_names, step):
    ...
    for fit in fits:
        ...
        stringvars = {'USER':user_name,'TIME':time_now,'CHOICE':'', 'NUMBERVARS':4-which_solution,'USER': user, 'step': step} 
        ...
        # get Id number. Hack for now, just assume one fit

    class Photfit(object):
    ...
    c.execute("select id, TIME from photometry_db where cluster='%s' and fitID='%s'" % (var_names['cluster'], var_names['fitID']))
    fits = [Photfit(id, time) for id, time in c.fetchall()]
    fits.sort()
    fit_id = fits[-1].id

    photometry_db.updateCalibration(cluster=var_names['cluster'], effectiveFilter=var_names['fitID'], fitId = fit_id, fitTable='photometry_db')
...
def usage():
    ...
    print "    -f, --fitID=STRING        unique id for fit recovery"
    print
    print "Author:"
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],
            ...
            "color=", "output=", "label=","sigmareject=","step=","cluster=","bandcomp=","color1=","color2=","nonint=","fitID="])
    except getopt.GetoptError:
        usage() 
        sys.exit(2)
    print sys.argv[1:]

    infile = night = extcoeff = colcoeff = outfile = label = sigmareject = step = cluster = color1which = color2which = bandcomp = nonint = fitID = None
    for o, a in opts:
        if o in ("-i", "--input"):
            infile = a
        elif o in ("-o", "--output"):
    ...
    print cluster,infile,outfile,extcoeff,sigmareject,step,bandcomp,color1which,color2which, fitID
    run_main(cluster,infile,outfile,extcoeff,sigmareject,step,bandcomp,color1which,color2which,nonint,label, fitID)
