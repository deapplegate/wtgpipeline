...
def make_database_stars():

    db2 = MySQLdb.connect(db='subaru', user='weaklensing', passwd='darkmatter', host='ki-sr01')
    c = db2.cursor()

    command = 'drop table if exists sdsstwomass'
    c.execute(command)

    command = 'create table if not exists sdsstwomass ( fieldid bigint, psfmagu float(10,4), psfmagg float(10,4), psfmagr float(10,4), psfmagi float(10,4), psfmagz float(10,4), psfmagj float(10,4), psfmagerru float(10,4), psfmagerrg float(10,4), psfmagerrr float(10,4), psfmagerri float(10,4), psfmagerrz float(10,4), psfmagerrj float(10,4), id bigint not null auto_increment, primary key (id))'
    ...
    command = "load data local infile '/nfs/slac/g/ki/ki04/pkelly/SLR/sdssall_pkelly50_0.csv' into table sdsstwomass fields terminated by ',' lines terminated by '\n' ignore 1 lines"
    print command
    c.execute(command)
    ...
    while loop:        
        ...
        if 1:
            ...
            if False: #len(results) > 0:
                commandst = 'update sdssdust set slrstatus="not ready" where fieldid="' + dtop2['fieldid'] + '"'
                c.execute(commandst)                                             
            else:
                #os.system('mkdir -p ' + os.environ['sne'] + '/scamp/')
                import os

                commandst = 'update sdssdust set slrstatus="started ' + str(datetime.now()) + '" where fieldid="' + dtop2['fieldid'] + '"'
                c.execute(commandst)                                             
                ...
                if 1:
                    ...
                    command = 'python run_slr.py ' + dtop2['fieldid'] + ' ' + dtop2['run'] + ' ' + dtop2['rerun'] + ' ' + dtop2['camcol'] + ' ' + dtop2['field'] 
                    print command 
                    a = subprocess.call(command,shell=True)
                    if float(a) != 0: 
                        commandst = 'update sdssdust set slrstatus="failed ' + str(datetime.now()) + '" where fieldid="' + dtop2['fieldid'] + '"'
                    else:
                        commandst = 'update sdssdust set slrstatus="finished" where fieldid="' + dtop2['fieldid'] + '"'
                    c.execute(commandst)                                             
