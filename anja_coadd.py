...
WORKDIR=SUBARUDIR+'/'+cluster+'/PHOTOMETRY_R'
...
for filter in filters:
	...
	if len(glob(IMAGEDIR) ) == 0: 
        ...
        OUTPUT_WORKDIR=SUBARUDIR + '/' +cluster + '/' + '/PHOTOMETRY_R/'
	print IMAGEDIR
...
for filter in filters:
	...
	if worst_filter != filter:
		...
		input.write(tempim + "\n")
		img = OUTPUT_WORKDIR + '/' + filter + '.smooth.fits'
		img = img.replace("//","/")
		pp.write("del " + img + '\n')
		output.write(img + "\n")
...
#IMAGEDIR=SUBARUDIR + '/' +cluster + '/' + worst_filter + '/PHOTOMETRY_R/'
img1 = IMAGEDIR + '/' + IMAGE + '.fits'
img2 = OUTPUT_WORKDIR + '/' + filter + '.smooth.fits'
