...
cat ${TEMPDIR}/coaddimages_$$ |\
{
  ...
  do
    ...
    if [ "${BADCCD}" != "1" ] && [ "${ASTBAD}" != "1" ]; then
      ...
      ln -s ${file} ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.copy.fits
      cp /${MAIN}/${WEIGHTDIR}/${BASEWEIGHT}.weight.fits \
            ./coadd_${SCRIPT}/${BASEWEIGHT}${WBASE}.copy.weight.fits

      if [ "${EXTERNHEAD}" -eq 1 ]; then
        cp /${MAIN}/${SCIENCE}/${EXTERNHEADDIR}/${HEADBASE}.head \
          ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head
	if [ "$?" -gt "0" ]; then exit $? ; fi
        cp ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.head ./coadd_${SCRIPT}/${HEADBASE}${EXTEN}.copy.head
      fi
    else
...
${P_FIND} . -maxdepth 1 -name \*${EXTEN}.copy.fits >> ${TEMPDIR}/files_$$.list
