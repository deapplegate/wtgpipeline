...
def all(subarudir,cluster,DETECT_FILTER,aptype,magtype,location=None):
    ...
    if True:
        ...
        def f(x): return x!=hold_all and not (string.find(x,'-2-') != -1 and string.find(x,'10')!=-1) and not (string.find(x,'MEGAPRIME') != -1 and x[-1] == 'u') and not (string.find(x,'WHT') != -1 and string.find(x,'U') != -1) #and string.find(x,'W-J-B') == -1 and string.find(x,'Z') == -1 #and string.find(x,'10_3') == -1
...
#                 mfc='#3399ff', mec='#3399ff', ms=1, mew=1)
            ...
            if True:                                                                          
                ...
                bands = bands[residuals < 10]
                bands_err = bands_err[residuals < 10]
                locus_matrix = locus_matrix[residuals < 10]
                good = good[residuals < 10]
                end_of_locus = end_of_locus[residuals < 10]
            ...
            if number_good_stars > len(locus_matrix) or len(filter(lambda x: x is False,end_of_locus.tolist())) > 0 and itr < 3:
