...
#print com
crpix1 = 4500 #float(com[1])
crpix2 = 4500 #float(com[2])
crval1 = 109.44915 #float(com[3])
crval2 = 37.750041 #float(com[4])
cdelt1 = 5.611000E-05 #float(com[5])
cdelt2 = 5.611000E-05 #float(com[6])
...
query = "select ra,dec,raErr,decErr,z, zConf from SpecObj where   ra between " + str(ramin)[:8] + " and  " + str(ramax)[:8] + " and  dec between " + str(decmin)[:8] + " and " +str(decmax)[:8]  + " AND z < 1.5 " # AND flags & dbo.fPhotoFlags('BLENDED') = 0 "# AND   " + flags 
...
keys = ['SeqNr',['dec','Dec'],['ra','Ra'],'raErr','decErr','umag','gmag','rmag','imag','Bmag','Vmag','Rmag','Imag','zmag','uerr','gerr','rerr','ierr','Berr','Verr','Rerr','Ierr','zerr','umg','gmr','rmi','imz','BmV','VmR','RmI','Imz','umgerr','gmrerr','rmierr','imzerr','BmVerr','VmRerr','RmIerr','Imzerr','flags_u','flags_g','flags_r','flags_i','flags_z','A_WCS','B_WCS','THETAWCS','Flag',['ra','ALPHA_J2000'],['dec','DELTA_J2000']]
seqnr = 1
for els in range(len(data)): 
	flag = data[els]['clean']
	print flag
	if int(flag)==1 :# 1==1: #data[els].has_key('u'):
		print 'added'
		...
		data[els]['A_WCS'] = error #data[els]['Err'] #'0.0004'
		data[els]['B_WCS'] = error #data[els]['decErr'] #'0.0004'
		data[els]['THETAWCS'] = '0'
		data[els]['Flag'] = '0'
