...
clusterz = float(re.split('\s+',cluster_info)[1])
...
if True: #for cluster,clusterz,filter in list:
    if 1: #try:
        #if len(sys.argv) != 5:                                                                                             
        ...
        kappacut = (phot['BPZ_Z_B_MAX'] - phot['BPZ_Z_B_MIN'] < 2.0)* (E<1.4)*(phot['BPZ_ODDS'] > 0.6) * (phot['NFILT'] > 4) * sp.calcWLViolationCut(r, beta, sigma_v = 1300)
        ...
        ran = [[0.1,0.3],[0.3,0.5],[0.5,0.65],[0.65,0.8],[0.8,0.95],[0.95,1.2],[1.1,1.3]] #,[1.3,1.5],[1.5,2.0]] #,[1.3,1.5]] 
        ...
        for w,color in [[-1,'black']]: #[-0.5,'black'],[-1,'blue'],[-1.5,'yellow']]: #scipy.arange(-10,10,2):
            ...
            z = 1.0
            DS = dict_DCMR['%.2f' % z + '_' + '%.2f' % w + '_' + '%.2f' % WM] #str(float('%.2f' % z)) + '_' + str(w) + '_' + str(WM)]
            ylim = 2.25 * (p[0] * ( DS - d_cluster)/DS) / infinite
        ...
        pylab.text(0.05,0.90*(ylim + 0.5) - 0.5,cluster, fontsize=20)
        pylab.text(0.05,0.77*(ylim + 0.5) - 0.5,'FWHM %.1f"' % float(seeing))
        pylab.text(0.05,0.84*(ylim + 0.5) - 0.5,'%.0f Galaxies' % float(ngal))
        pylab.text(0.05,0.70*(ylim + 0.5) - 0.5,'$\chi^{2}_{\mathrm{red}}$ %.2f' % redchi)

        path = os.environ['sne'] + '/photoz/' + cluster + '/CWWSB_capak.list/'

        fn = path + 'shearratio.png'
        pylab.savefig(fn)

        groupf = os.environ['sne'] + '/photoz/shearratios/' + cluster + '_sr.pdf'
        pylab.savefig(groupf)

        print cluster + 'ACSratio.png'
