...
do
	filter=${pprun#2*_}
	run=${pprun%_*}
	#echo "cd /u/ki/awright/data/${run}_${filter}/SCIENCE" >> postH_setup.tmp
	...
	bsub -W 7000 -R rhel60 -o /u/ki/awright/data/batch_files/postH_preprocess/OUT-postH_${run}_${filter}_preprocess_step9.out \
			       -e /u/ki/awright/data/batch_files/postH_preprocess/OUT-postH_${run}_${filter}_preprocess_step9.err \
			       ./postH_${run}_${filter}_preprocess_step9.sh
	#echo "./get_error_log.sh /u/ki/awright/data/batch_files/postH_preprocess/OUT-postH_${run}_${filter}_preprocess_step9.out" >> adam_SHNT.sh
	#echo "vim /u/ki/awright/data/batch_files/postH_preprocess/OUT-postH_${run}_${filter}_preprocess_step9.err" >> adam_SHNT.sh
done
