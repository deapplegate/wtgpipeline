...
for letter in ['A','B','C','D','E','G1','G2']:
    obj_poly = 'reg' + letter + 'all.reg' #'M2243_obj_W-J-V.reg'
    bg_poly = 'reg' + letter + 'bg.reg' #'M2243_W-J-V.reg'
    ALPHA_J2000 = 340.82489
    DELTA_J2000 = -9.59580
    
    filters = ['W-C-RC'] #'B','W-J-V','W-C-RC','W-C-IC','W-S-Z+']
    ...
    for filter in filters:
        ...
        image_small = "/nfs/slac/g/ki/ki05/anja/proposals/chandra_ao11/hi-z_cluster/lenstool/image.fits" #/%(path)s/%(filter)s/PHOTOMETRY/coadd_small.fits" % params
        ...
        flag_small = '"a.fits"' #"/%(path)s/%(filter)s/PHOTOMETRY/coadd_small.flag.fits" % params
        #run('rm ' + flag_small + '; makesubimage -500 -500 1000 1000 -c < ' + flag_file + ' > ' + flag_small)

        for f in [obj_poly,bg_poly]:
        ...
        obj['mag'] = 0.38 + 27.0 - 2.5*math.log10(obj['flux'])
    
    
        obj['sb'] = 0.38 + 27.0 - 2.5*math.log10(obj['flux']/(PIXSCALE**2.*obj['pix']))
        
        print letter + ': object flux', obj['flux'], 'mag', obj['mag'], 'error', obj['err'], 'GAIN', GAIN, 'SB', obj['sb'], 'PIXSCALE', PIXSCALE

        rec.write(letter + ': object flux '+ str(obj['flux']) + ' mag '+ str(obj['mag']) + ' error '+ str(obj['err']) + ' GAIN '+ str(GAIN) + ' SB '+ str(obj['sb']) + ' PIXSCALE '+ str(PIXSCALE) + '\n')
