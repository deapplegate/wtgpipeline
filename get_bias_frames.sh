...
if [ ${periodWarn} -eq 1 ]; then
    ...
    #adam-BL#./BonnLogger.py comment 'Warning: Different bias periods detected'
fi
...
for ((i=1;i<=${NCHIPS};i+=1)); do
	...
	#adam-BL# log_status 2 "Bad Bias Linking: Chip $i"
	echo "adam-look | error: Bad Bias Linking: Chip $i"
	exit 2
    fi
done
