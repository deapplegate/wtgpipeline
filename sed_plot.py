...
def plot_bpz_probs(ids,file,outfile):    
    import re, scipy
...
def run(cluster):
    ...
    if False:
        ...
        from datetime import datetime
        t2 = datetime.now()
        pagemain.write(t2.strftime("%Y-%m-%d %H:%M:%S") + '<ul>')
        ...
        for key in corrections.keys():
            if key != 'ID' and key != 'Z_S':
                ...
                print stars_dict.keys()
                if stars_dict.has_key(key):
                    nums += key + ' ' + str(stars_dict[key]) + '<br>\n'
        pagemain.write('</ul>' + nums +  '</ul>' + ims)
        pagemain.close()
    ...
    ffile = os.environ['sne'] + '/photoz/' + cluster + '/fix.tif'
    ...
    os.system('convert ' + os.environ['sne'] + '/photoz/' + cluster + '/fix.tif ' + os.environ['sne'] + '/photoz/' + cluster + '/fix.jpg')
    
    print ffile
    
    import Image
    im = Image.open(os.environ['sne'] + '/photoz/' + cluster + '/fix.jpg')
    ...
    import scipy
    set = [int(e[:-1]) for e in open(redseq,'r').readlines()][0:20]
    ...
    plot_bpz_probs(set, probs + '.tab',file + '_probs')
