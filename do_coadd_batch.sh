...
if [ "${INSTRUMENT}" == "SUBARU" ]; then
  CONFIG=`grep 'config=' ${INSTRUMENT}.ini | sed 's/config=//g'`
fi
...
##############################
  ...
  ./prepare_coadd_swarp.sh -m ${SUBARUDIR}/${cluster}/${filter} \
    ...
    ./parallel_manager.sh ./resample_coadd_swarp_chips_para.sh ${SUBARUDIR}/${cluster}/${filter} SCIENCE "${ending}.sub" ${cluster}_gabodsid${GABODSID} ${REDDIR} ${cluster}_all
...
### new rotation coadd: split by both night and rotation
  ...
  ./prepare_coadd_swarp_chips.sh -m ${SUBARUDIR}/${cluster}/${filter} \
    ...
    ./parallel_manager.sh ./resample_coadd_swarp_chips_para.sh ${SUBARUDIR}/${cluster}/${filter} SCIENCE "${ending}.sub" ${cluster}_gab${GABODSID}-rot${ROTATION} ${REDDIR} ${cluster}_all
