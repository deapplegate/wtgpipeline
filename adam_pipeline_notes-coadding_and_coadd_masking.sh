...
for fl in `\ls SUPA00329*_8.reg` ; do ochip=`basename ${fl} _8.reg` ; cp ${fl} ${ochip}_3.reg ; done
for fl in `\ls SUPA00329*_8.reg` ; do ochip=`basename ${fl} _8.reg` ; cp ${fl} ${ochip}_7.reg ; done
for fl in `\ls SUPA00329*_4.reg` ; do ochip=`basename ${fl} _4.reg` ; cp ${fl} ${ochip}_5.reg ; done
for fl in `\ls SUPA00329*_4.reg` ; do ochip=`basename ${fl} _4.reg` ; cp ${fl} ${ochip}_9.reg ; done
for fl in `\ls SUPA00329*_4.reg` ; do ochip=`basename ${fl} _4.reg` ; cp ${fl} ${ochip}_10.reg ; done
...
#vim ./adam_do_photometry.sh
export cluster=RXJ2129 ; export detect_filter=${filter};export lensing_filter=${filter} ;export ending=OCFSIR ; export config="10_3"
./adam_do_photometry.sh ${cluster} ${detect_filter} ${lensing_filter} aper PHOTO MERGE STARS BIGMACSCALIB BIGMACSAPPLY 2>&1 | tee -a OUT-adam_do_photometry_${cluster}_allmodes.log
